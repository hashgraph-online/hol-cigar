...
class ConsumerTests(unittest.TestCase):
    ...
    def test_noncanonical_duplicate_and_oracle_assignments_fail_closed(self) -> None:
        path, payload = self.write_assignment(
            "assignment.json", "candidate", GIT_B
        )
    ...
    def test_incomplete_tampered_and_duplicate_observations_fail_closed(self) -> None:
        ...
        assignment, assignment_bytes = load_assignment(
            candidate_path, self.registry
        )
        executable_digest = "1220" + hashlib.sha256(
            self.consumer.read_bytes()
        ).hexdigest()
        observation = next(
            value
    ...
    def test_mismatched_pair_and_oversized_output_fail_closed(self) -> None:
        champion, _ = self.write_assignment(
            "champion.json", "champion", GIT_A
        )
        ...
        flood.chmod(0o555)
        with mock.patch(
            "tools.refinement.consumer.MAX_STDOUT_BYTES", 1024
    ...
    def test_timeout_stderr_and_descendant_leaks_fail_closed(self) -> None:
        ...
        candidate, _ = self.write_assignment(
            "candidate.json", "candidate", GIT_B
        )
        common = {
            "champion_assignment_path": champion,
    ...
    def test_real_rust_consumer_replays_without_canary_disclosure(self) -> None:
        ...
        for payload in (
            ...
            canonical.canonical_bytes(
                {**champion_value, "archive_digest": MH}
            ),
            canonical.canonical_bytes(
                {**champion_value, "oracle": CANARY}
            ),
        ):

...
class CommandControllerTests(unittest.TestCase):
    ...
    def test_refinement_contract_gate_uses_the_complete_pytest_suite(self) -> None:
        self.assertEqual(
            default_registry().get("refinement-contracts").argv,
            (
                sys.executable,
                "-m",
                "pytest",
    ...
    def test_launcher_preserves_a_validated_executable_alias(self) -> None:
        target = self.root / "role-target"
        target.write_text(
            '#!/bin/sh\ntest "$(basename "$0")" = "role-alias"\n',
            encoding="utf-8",
        )
        target.chmod(0o700)
        alias = self.root / "role-alias"
        alias.symlink_to(target)
        registry = CommandRegistry((CommandSpec("alias-role", (str(alias),), 2),))

        result = run_named(
            registry,
            "alias-role",
            cwd=self.cwd,
            state=self.root / "alias-role-state",
        )

        self.assertEqual(result["status"], "passed")
        self.assertEqual(result["executable_invocation_path"], str(alias))
        self.assertEqual(result["executable_path"], str(target))

    def test_launcher_preserves_the_active_python_environment(self) -> None:
        expected_prefix = sys.prefix
        result = self.run_code(
            "import sys; raise SystemExit(sys.prefix != " + repr(expected_prefix) + ")"
        )
        self.assertEqual(result["status"], "passed")
    ...
    def test_isolated_environment_reuses_only_a_validated_uv_cache(self) -> None:
        cache = self.root / "uv-cache"
        cache.mkdir(mode=0o700)
        previous = os.environ.get("UV_CACHE_DIR")
        try:
            os.environ["UV_CACHE_DIR"] = str(cache)
            environment = sanitized_environment(self.root / "uv-cache-state")
            self.assertEqual(environment["UV_CACHE_DIR"], str(cache))
            self.assertEqual(environment["HOME"], str(self.root / "uv-cache-state"))

            cache.chmod(0o777)
            unsafe = sanitized_environment(self.root / "unsafe-uv-cache-state")
            self.assertNotIn("UV_CACHE_DIR", unsafe)
        finally:
            cache.chmod(0o700)
            if previous is None:
    ...
    @unittest.skipIf(os.name == "nt", "default Windows uv cache uses LOCALAPPDATA")
    def test_isolated_environment_finds_uv_cache_before_replacing_home(self) -> None:
        fake_home = self.root / "fake-home"
        cache = fake_home / ".cache" / "uv"
        cache.mkdir(mode=0o700, parents=True)
        previous_home = os.environ.get("HOME")
        previous_cache = os.environ.pop("UV_CACHE_DIR", None)
        try:
            os.environ["HOME"] = str(fake_home)
            environment = sanitized_environment(self.root / "derived-uv-cache-state")
            self.assertEqual(environment["UV_CACHE_DIR"], str(cache))
            self.assertEqual(
                environment["HOME"], str(self.root / "derived-uv-cache-state")
            )
        finally:
            ...
            if previous_cache is not None:
                os.environ["UV_CACHE_DIR"] = previous_cache

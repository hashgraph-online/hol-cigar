...
class R10OperationsTests(unittest.TestCase):
    ...
    def test_new_schemas_are_closed_and_configs_bind_the_operations_policy(
        ...
        self.assertEqual(
            policy["policy_id"],
            "1220ced894ec6254a195be1e1a3556bc9f0450f9f6d8037bb02f2eb34dc166a82839",
        )
        for filename in (
            "default-v1.toml",
            "fast-v1.toml",
    ...
    def test_quota_reservations_enforce_daily_provider_and_concurrency_limits(
        ...
        cli = quota.reserve(
            utc_day="2026-07-27",
            provider_id="codex-cli-tools-v1",
            reservation_id="codex-cli-one",
            requested={
                "input_tokens": 240_000,
                "output_tokens": 80_000,
                "cost_microusd": 30_000_000,
                "compute_milliseconds": 4_200_000,
            },
        )
        self.assertEqual(cli["kind"], "reserved")
        quota.finish(
            "codex-cli-one",
            actual={
        ...
        self.assertEqual(len(quota.replay()), 4)
    ...
    def test_workflow_audit_proves_lane_separation_and_rejects_pr_secrets(
        ...
        self.assertEqual(by_name["refinement-shadow.yml"]["secret_handles"], [])
        self.assertEqual(
        ...
        fast.write_text(
            fast.read_text(encoding="utf-8").replace(
                "\n# ${{ secrets.CIGAR_SHADOW_ATTESTATION_KEY }}\n", "\n"
            ),
            encoding="utf-8",
        )
        shadow = fixture / ".github" / "workflows" / "refinement-shadow.yml"
        with shadow.open("a", encoding="utf-8") as stream:
            stream.write("\n# ${{ secrets.CIGAR_SHADOW_ATTESTATION_KEY }}\n")
        with self.assertRaisesRegex(WorkflowAuditError, "candidate-ref shadow workflow"):
            audit(fixture, copied_policy)
        canonical = "${{ secrets.CIGAR_SHADOW_ATTESTATION_KEY }}"
        unsupported_expressions = (
            "${{\n SeCrEtS [ 'CIGAR_SHADOW_ATTESTATION_KEY' ]\n}}",
            "${{ toJSON(secrets) }}",
            "${{ secrets }}",
            "${{ toJSON(\n SeCrEtS\n) }}",
            "${{ format('{{Hello {0}!}}', toJSON(secrets)) }}",
        )
        for expression in unsupported_expressions:
            with self.subTest(expression=expression):
                ...
                with self.assertRaisesRegex(
                    WorkflowAuditError, "unsupported secret expression"
                ):
                    audit(fixture, copied_policy)
                shadow.write_text(
                    shadow.read_text(encoding="utf-8").replace(
        ...
        encoded_expressions = (
            r"${{ \u0073ecrets.CIGAR_SHADOW_ATTESTATION_KEY }}",
            "$\\\n{{ secrets.CIGAR_SHADOW_ATTESTATION_KEY }}",
            "${{ format(''{{Hello {0}!}}'', toJSON(secrets)) }}",
        )
        for expression in encoded_expressions:
            with self.subTest(expression=expression):
                ...
                with self.assertRaisesRegex(
                    WorkflowAuditError, "encoded secret expression"
                ):
                    audit(fixture, copied_policy)
                shadow.write_text(
                    shadow.read_text(encoding="utf-8").replace(
        ...
        self.assertEqual(audit(fixture, copied_policy)["status"], "passed")
        safe_shadow = shadow.read_text(encoding="utf-8")
        secret_declarations = (
            "    secrets: inherit\n",
            '    "secrets": "inherit"\n',
            r'    "secr\u0065ts": inherit' + "\n",
        ...
        for declaration in secret_declarations:
            with self.subTest(declaration=declaration):
                shadow.write_text(
                    safe_shadow
                    + "\n  inherited-secrets:\n"
                    + "    uses: example/refinement/.github/workflows/reusable.yml@"
                    + f"{'1' * 40}\n"
                    + declaration,
                    encoding="utf-8",
                )
                with self.assertRaisesRegex(
                    WorkflowAuditError, "unsupported secret declaration"
                ):
                    audit(fixture, copied_policy)
        shadow.write_text(
            safe_shadow
            + (
                ...
                "    uses: example/refinement/.github/workflows/reusable.yml@"
                f"{'1' * 40}\n"
                "    x-key: &secret-key secrets\n"
                "    ? *secret-key\n"
                "    : inherit\n"
            ),
            encoding="utf-8",
        )
        with self.assertRaisesRegex(WorkflowAuditError, "unsupported YAML alias"):
            audit(fixture, copied_policy)
        shadow.write_text(
            safe_shadow
            + (
        ...
        with self.assertRaisesRegex(
            WorkflowAuditError, "mutation/publication authority"
        ):
            audit(fixture, copied_policy)
        shadow.write_text(
            safe_shadow
            + (
                ...
                "    timeout-minutes: 1\n"
                "    steps:\n"
                '      - "uses": example/refinement-action@main\n'
            ),
            encoding="utf-8",
        )
        with self.assertRaisesRegex(WorkflowAuditError, "not commit-pinned"):
            audit(fixture, copied_policy)
        shadow.write_text(
            safe_shadow.replace(
                "\npermissions:\n",
        ...
        with self.assertRaisesRegex(WorkflowAuditError, "unsupported trigger"):
            audit(fixture, copied_policy)
    ...
    def test_workflow_policy_and_managed_workflows_are_stable(self) -> None:
        ...
        shadow = (ROOT / ".github/workflows/refinement-shadow.yml").read_text(
            encoding="utf-8"
        )
        self.assertNotIn("${{ secrets.", shadow)
        self.assertNotIn("actions/checkout", shadow)
        self.assertNotIn("--attestation-key", shadow)
        self.assertNotIn("--gate-key", shadow)

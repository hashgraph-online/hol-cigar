...
class CorpusQualificationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.packs = {
            role: load(CORPUS / "development" / f"{role}.json") for role in PACK_ROLES
        }
        cls.maps = {
    ...
    def test_every_stratum_has_thirty_independent_tasks(self) -> None:
        counts = {
            item["stratum"]: item["count"] for item in self.manifest["stratum_counts"]
        }
    ...
    def test_annotation_agreement_and_abstention_quarantine_policy(self) -> None:
        ...
        self.assertEqual(self.manifest["qualification"]["agreement_ppm"], 1_000_000)
    ...
    def test_baseline_cigar_proxy_and_oracle_selections_replay(self) -> None:
        ...
        self.assertTrue(
            all(item["technically_executable"] for item in summaries.values())
        )
    ...
    def test_production_rust_cigar_is_executable_without_canary_disclosure(
        self,
    ) -> None:
        ...
        expected_digest = os.environ.get("CIGARBENCH_CONSUMER_DIGEST")
        if expected_digest is None:
            self.fail("set CIGARBENCH_CONSUMER_DIGEST to the exact selected executable")
        if (
            len(expected_digest) != 68
            or not expected_digest.startswith("1220")
            or any(character not in "0123456789abcdef" for character in expected_digest)
        ):
            self.fail("CIGARBENCH_CONSUMER_DIGEST must be a SHA-256 multihash")
        ...
        self.assertEqual(result["consumer_digest"], expected_digest)
    ...
    def test_canaries_licenses_and_pack_contracts_are_complete(self) -> None:
        ...
        self.assertEqual(self.manifest["qualification"]["setup_smoke_passed"], 270)
        self.assertEqual(

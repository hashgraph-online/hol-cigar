"""Paired, attested qualification for Honey and H1 intelligence profiles."""
...
PROFILE_H2 = "balanced.v2-candidate.2"
PROFILE_V3 = "balanced.v3"
SUPPORTED_PROFILES = frozenset({PROFILE_V1, PROFILE_V2, PROFILE_H2, PROFILE_V3})
ALLOWED_PROFILE_TRANSITIONS = frozenset(
    {
        (PROFILE_V1, PROFILE_V2),
        (PROFILE_V2, PROFILE_V2),
        (PROFILE_V2, PROFILE_H2),
        (PROFILE_H2, PROFILE_V3),
    }
)
# Official qualification inherits the immutable corpus contract. A smaller explicit
# value remains available for non-promotion stress runs.
...
def _qualification_token_budget(
    records: dict[str, dict[str, dict[str, Any]]],
    task_ids: Sequence[str],
    requested: int | None,
) -> int:
    """Resolve one cohort budget without silently overriding frozen task contracts."""
    contract_budgets = {
        records["tasks"][task_id]["contract"]["token_budget"] for task_id in task_ids
    }
    if len(contract_budgets) != 1:
        raise IntelligenceError(
            "qualification tasks do not share one frozen token budget"
        )
    contract_budget = contract_budgets.pop()
    if not isinstance(contract_budget, int) or isinstance(contract_budget, bool):
        raise IntelligenceError("qualification task token budget is invalid")
    selected = contract_budget if requested is None else requested
    if selected <= 0 or selected > contract_budget:
...
def _external_output(path: Path, repository_root: Path) -> Path:
    if not path.is_absolute() or path.is_symlink() or path.exists():
        raise IntelligenceError(
            "evidence directory must be absolute, new, and non-symlink"
        )
...
def _verify_sealed_record(
    value: dict[str, Any], *, identity_field: str, key: bytes, key_fingerprint: str
) -> bool:
    attestation = value.get("attestation")
    if not isinstance(attestation, dict) or not isinstance(attestation.get("mac"), str):
        return False
    if attestation.get("key_fingerprint") != key_fingerprint:
        return False
    unsigned_mac = dict(value)
    unsigned_attestation = dict(attestation)
    supplied_mac = unsigned_attestation.pop("mac")
    unsigned_mac["attestation"] = unsigned_attestation
    identified = dict(unsigned_mac)
    claimed_identity = identified.pop(identity_field, None)
    if claimed_identity != identity(identified):
        return False
    expected_mac = hmac.new(
        key, canonical_bytes(unsigned_mac), hashlib.sha256
    ).hexdigest()
    return hmac.compare_digest(supplied_mac, expected_mac)


def _validate_gate_attachment(
    payload: bytes,
    *,
    source: dict[str, str],
    plan_id: str,
    ...
    try:
        ...
        raise IntelligenceError("gate attachment is not strict bounded JSON") from error
    if not isinstance(value, dict) or canonical_bytes(value) != payload:
        raise IntelligenceError("gate attachment is not exact passed source evidence")
    try:
        registry.validate("intelligence-gate-evidence-v2.schema.json", value)
    except ValueError as error:
        raise IntelligenceError("gate attachment violates its closed schema") from error
    body = dict(value)
    claimed_evidence_id = body.pop("evidence_id")
    receipts = value["receipts"]
    if (
        claimed_evidence_id != identity(body)
        or value["source"] != source
        or value["plan_id"] != plan_id
        or value["build_set_id"] != build_set_id
        or value["policy_digest"] != policy_digest
        or [item["gate_id"] for item in receipts] != sorted(set(required_checks))
    ):
        raise IntelligenceError("gate attachment is not exact passed source evidence")
    for receipt in receipts:
        command_results = receipt["command_results"]
        attachments = receipt["attachment_digests"]
        if (
            receipt["source"] != source
            or receipt["plan_id"] != plan_id
            or receipt["build_set_id"] != build_set_id
            or receipt["policy_digest"] != policy_digest
            or [item["command_id"] for item in command_results]
            != sorted({item["command_id"] for item in command_results})
            or attachments != sorted(set(attachments))
            or any(
                item["result_id"]
                != identity(
                    {key: field for key, field in item.items() if key != "result_id"}
                )
                for item in command_results
            )
            or not _verify_sealed_record(
                receipt,
            ...
            raise IntelligenceError("gate receipt authenticity is invalid")
    return value
...
def _evidence_bridge(
    ...
    for file in fixture["archive"]["files"]:
        ...
        if digest in digest_to_evidence:
            raise IntelligenceError(
                "evidence bridge contains an ambiguous content digest"
            )
...
def _run_verifier(
    *,
    ...
    verifier = (environment_root / oracle["deterministic_verifier"]).resolve(
        strict=True
    )
    ...
    if [item["check_id"] for item in checks] != sorted(
...
def _derive_metrics(
    *,
    ...
    critical = {
        item["evidence_id"]: item["weight"] for item in oracle["critical_evidence"]
    }
...
def _evaluate_observation(
    *,
    ...
    selected_profile = profile_id or (
        PROFILE_V1 if observation["treatment"] == "champion" else PROFILE_V2
    )
    if selected_profile not in SUPPORTED_PROFILES:
        raise IntelligenceError("evaluation intelligence profile is unknown")
    body = {
...
def _failed_metrics(
    *, task: dict[str, Any], oracle: dict[str, Any], failure: dict[str, Any]
) -> list[dict[str, Any]]:
    critical_total = sum(item["weight"] for item in oracle["critical_evidence"])
    required_claims = oracle["required_claims"]
    values = {
        "verified_task_success": _metric("verified_task_success", 0),
        "critical_context_recall": _metric(
            "critical_context_recall", 0, critical_total, applicable=critical_total > 0
        ),
        "evidence_token_precision": _metric("evidence_token_precision", 0, 1),
        "evidence_item_precision": _metric("evidence_item_precision", 0, 1),
        "citation_recall": _metric("citation_recall", 0, applicable=False),
        "citation_precision": _metric("citation_precision", 0, applicable=False),
        "unsupported_claim_rate": _metric(
            "unsupported_claim_rate", 0, applicable=False
        ...
        "first_useful_evidence_rank": _metric(
            "first_useful_evidence_rank", 10_000, applicable=critical_total > 0
        ),
        "evidence_sufficiency": _metric(
            "evidence_sufficiency",
            0,
        ...
        "selected_provenance_coverage": _metric("selected_provenance_coverage", 0, 1),
        "authorization_violations": _metric("authorization_violations", 0),
        "prohibited_materialized_tokens": _metric("prohibited_materialized_tokens", 0),
        "digest_mismatches": _metric("digest_mismatches", 0),
        "unsafe_effect_retries": _metric("unsafe_effect_retries", 0),
        "budget_overflow": _metric("budget_overflow", 0),
        "physical_input_tokens": _metric("physical_input_tokens", 0),
        "cache_read_tokens": _metric("cache_read_tokens", 0),
        "cache_write_tokens": _metric("cache_write_tokens", 0),
        "output_tokens": _metric("output_tokens", 0),
        "latency_ms": _metric("latency_ms", failure["duration_ms"]),
        "cpu_ms": _metric("cpu_ms", 0, applicable=False),
        "peak_rss_bytes": _metric("peak_rss_bytes", 0, applicable=False),
        "cost_usd": _metric("cost_usd", 0),
        "handoffs": _metric("handoffs", 0),
        "effects": _metric("effects", 0),
        "replay_dispatches": _metric("replay_dispatches", 0),
        "human_agreement": _metric("human_agreement", 0, applicable=False),
    }
    if set(values) != REQUIRED_METRICS:
        raise IntelligenceError("failed-treatment metric inventory is incomplete")
    return [values[name] for name in sorted(values)]
...
def _failed_evaluation(
    *,
    ...
    if profile_id not in SUPPORTED_PROFILES:
        raise IntelligenceError("failed evaluation intelligence profile is unknown")
    body = {
        "schema_version": "cigar.honey-treatment-failure-evaluation.v1",
        "task_id": task["task_id"],
        "task_lineage_id": task["task_lineage_id"],
        "stratum": task["stratum"],
        "seed_index": seed_index,
        ...
        "failure": failure,
        "metrics": _failed_metrics(task=task, oracle=oracle, failure=failure),
    }
    value = _seal(
        body,
        identity_field="evaluation_id",
    ...
    registry.validate("honey-treatment-failure-evaluation-v1.schema.json", value)
    return value
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(dir=scratch, prefix=f"{pair_id}-") as raw:
        ...
        if result["pair_id"] != pair_id:
            raise IntelligenceError("profile three-way result changed pair identity")
        ...
        if set(observations) != {
            ...
            raise IntelligenceError("profile observation/outcome inventory drifted")
        evaluations = {}
        profiles = {
            "honey": PROFILE_V1,
            "champion": champion_profile,
...
def qualify(
    *,
    ...
    champion_profile: str = PROFILE_V1,
    candidate_profile: str = PROFILE_V2,
    ...
    gate_key_path = gate_key_path.resolve(strict=True)
    ...
    if evidence_class not in {"development", "shadow"}:
        raise IntelligenceError(
            "only development and shadow qualification are supported"
        )
    ...
    if (token_budget is not None and token_budget <= 0) or seeds <= 0 or seeds > 16:
        raise IntelligenceError("profile qualification bounds are invalid")
    if allow_dirty:
        raise IntelligenceError("exact-source qualification requires a clean checkout")
    if (champion_profile, candidate_profile) not in ALLOWED_PROFILE_TRANSITIONS:
        raise IntelligenceError(
            "profile qualification transition is not sequentially allowed"
        )
    source = _git_source(repository_root, require_clean=True)
    key, key_fingerprint = _attestation_key(key_path, repository_root)
    gate_key, gate_key_fingerprint = _attestation_key(gate_key_path, repository_root)
    if gate_key_fingerprint == key_fingerprint:
        raise IntelligenceError(
            "gate and qualification authority must use distinct keys"
        )
    try:
        plan, build_receipt, executables = load_source_consumers(
            repository_root=repository_root,
            plan_path=plan_path,
            build_root=build_root,
        )
    except SourceBuildError as error:
        raise IntelligenceError("exact-source build custody is invalid") from error
    profiles = plan.get("intelligence_profiles")
    if (
        plan.get("cycle") != "cycle-b"
        or not isinstance(profiles, dict)
        or source != plan["product_sources"]["candidate"]
        or profiles.get("honey") != PROFILE_V1
        or profiles.get("champion") != champion_profile
        or profiles.get("candidate") != candidate_profile
    ):
        raise IntelligenceError(
            "qualification does not match its sequential Cycle B plan"
        )
    sources = {
    ...
    gate_evidence = _validate_gate_attachment(
        gate_attachment,
        source=source,
        plan_id=plan["plan_id"],
        build_set_id=build_receipt["build_set_id"],
        policy_digest=policy_digest,
        required_checks=policy["tier1_external_checks"],
        key=gate_key,
        key_fingerprint=gate_key_fingerprint,
        registry=registry,
    )
    ...
    registry_path = (
        repository_root / "refinement/profiles/intelligence-profiles.v1.json"
    )
    ...
    registered_profiles = {item["profile_id"] for item in profile_registry["profiles"]}
    if {champion_profile, candidate_profile} - registered_profiles:
        raise IntelligenceError(
            "qualification profile is absent from the signed registry"
        )
    ...
    if sources["honey"] != {
        ...
        raise IntelligenceError("Honey artifact source does not match its anchor")
    ...
    for task_id in task_ids:
        ...
        for seed_index in range(seeds):
            honey_evaluation, champion, candidate, pair_id = _pair(
                repository_root=repository_root,
            ...
            for evaluation in (honey_evaluation, champion, candidate):
                _write_canonical(
            ...
            pairs.append(
                {
                    ...
                    "honey": {
                        "evaluation_digest": honey_evaluation["evaluation_id"],
                        "metrics": honey_evaluation["metrics"],
                    },
                }
            )
    ...
    pair_ids_digest = identity(sorted(item["pair_id"] for item in pairs))
    source_after = _git_source(repository_root, require_clean=True)
    if source_after != source:
        raise IntelligenceError("candidate source changed during qualification")
    tier0_evidence = {
        "artifact-digests": build_receipt["build_set_id"],
        "assignment-balance": pair_ids_digest,
        "attachment-integrity": gate_evidence["evidence_id"],
        "benchmark-immutability": manifest["manifest_id"],
        "calibration": identity(manifest["qualification"]),
        "clean-source-after": identity(source_after),
        "clean-source-before": identity(source),
        "contamination": evaluation_ids_digest,
        "evaluator-attestation": key_fingerprint,
        "exact-pins": plan["plan_id"],
        "observation-chain": evaluation_ids_digest,
        "source-identity": identity(sources),
    }
    if set(tier0_evidence) != set(policy["tier0_checks"]):
        raise IntelligenceError("Tier-0 policy and derived evidence inventory disagree")
    tier0_body = {
        "schema_version": "cigar.intelligence-tier0-attachment.v1",
        "plan_id": plan["plan_id"],
        "build_set_id": build_receipt["build_set_id"],
        "manifest_id": manifest["manifest_id"],
        "gate_evidence_id": gate_evidence["evidence_id"],
        "evaluation_ids_digest": evaluation_ids_digest,
        "pair_ids_digest": pair_ids_digest,
        "sources": sources,
        "consumer_digests": consumer_digests,
        "checks": [
            {"check_id": check, "evidence_digest": tier0_evidence[check]}
            for check in sorted(tier0_evidence)
        ],
        "status": "passed",
    }
    tier0_attachment = {
        **tier0_body,
        "attachment_id": identity(tier0_body),
    }
    registry.validate("intelligence-tier0-attachment-v1.schema.json", tier0_attachment)
    _write_canonical(output / "tier0-attachment.json", tier0_attachment)
    checks0 = [
        {
            "check_id": check,
            "passed": True,
            "attachment_digest": tier0_attachment["attachment_id"],
        }
        for check in policy["tier0_checks"]
    ]
    receipts = {item["gate_id"]: item for item in gate_evidence["receipts"]}
    checks1 = [
        {
            "check_id": check,
            "passed": True,
            "attachment_digest": receipts[check]["receipt_id"],
        }
        for check in policy["tier1_external_checks"]
    ]
    trial_id = f"trial-intelligence-{sources['candidate']['revision'][:12]}"
    comparison_body = {
    ...
    qualification_body = {
        ...
        "evaluation_ids_digest": evaluation_ids_digest,
        "gate_evidence_id": gate_evidence["evidence_id"],
        "tier0_attachment_id": tier0_attachment["attachment_id"],
    ...
    registry.validate(
        "intelligence-profile-qualification-v1.schema.json", qualification
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--evidence-class", required=True, choices=("development", "shadow")
    )
    ...
    parser.add_argument(
        "--champion-profile", choices=sorted(SUPPORTED_PROFILES), default=PROFILE_V1
    )
    parser.add_argument(
        "--candidate-profile", choices=sorted(SUPPORTED_PROFILES), default=PROFILE_V2
    )

...
THREE_WAY_TREATMENTS = ("honey", "champion", "candidate")
PROFILE_TRANSITIONS = frozenset(
    {
...
SAFE_FAILURE = re.compile(
    rb"^cigarbench consumer rejected the observation: ([A-Za-z0-9:_-]{1,128})\n$"
)
MEDIA_TYPES = {
    ".txt": "text/plain",
    ".md": "text/markdown",
...
class ConsumerProcessFailure(ConsumerError):
    """A bounded consumer exited unsuccessfully without breaking controller custody."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__("consumer process failed")
        matched = SAFE_FAILURE.fullmatch(stderr)
        self.outcome = {
            "category": "process-exit",
            "exit_code": exit_code,
            "failure_code": (
                matched.group(1).decode("ascii", errors="strict") if matched else None
            ),
            "stdout_bytes": len(stdout),
            "stdout_digest": multihash_bytes(stdout),
            "stderr_bytes": len(stderr),
            "stderr_digest": multihash_bytes(stderr),
...
def _validate_archive(assignment: dict[str, Any], registry: SchemaRegistry) -> None:
    ...
    for entry in value["files"]:
        ...
        try:
            ...
            raise ConsumerError(
                "fixture archive content is not strict base64url"
            ) from error
...
def _validate_pair(champion: dict[str, Any], candidate: dict[str, Any]) -> None:
...
def _validate_three_way(assignments: dict[str, dict[str, Any]]) -> None:
    if tuple(assignments) != THREE_WAY_TREATMENTS:
        raise ConsumerError("three-way assignment inventory is invalid")
    reference = assignments["honey"]
    for treatment in THREE_WAY_TREATMENTS:
        assignment = assignments[treatment]
        if assignment["treatment"] != treatment:
            raise ConsumerError("three-way assignment has the wrong treatment")
        if assignment.get("intelligence_profile") != "balanced.v1":
            raise ConsumerError("three-way assignment is not frozen balanced.v1")
        for field in PAIR_INVARIANT_FIELDS:
            if assignment[field] != reference[field]:
                raise ConsumerError(
                    "three-way assignments differ outside treatment/source"
                )
    sources = {
        (assignment["source"]["revision"], assignment["source"]["tree"])
        for assignment in assignments.values()
    }
    if len(sources) != len(THREE_WAY_TREATMENTS):
        raise ConsumerError("three-way assignments do not bind three distinct sources")


def _validate_profile_three_way(assignments: dict[str, dict[str, Any]]) -> None:
    if tuple(assignments) != THREE_WAY_TREATMENTS:
        raise ConsumerError("profile three-way assignment inventory is invalid")
    reference = assignments["honey"]
    profiles = {
        treatment: assignments[treatment].get("intelligence_profile")
        for treatment in THREE_WAY_TREATMENTS
    }
    if (
        profiles["honey"] != "balanced.v1"
        or (profiles["champion"], profiles["candidate"]) not in PROFILE_TRANSITIONS
    ):
        raise ConsumerError("profile three-way transition is not sequentially allowed")
    for treatment in THREE_WAY_TREATMENTS:
        assignment = assignments[treatment]
        if assignment["treatment"] != treatment:
            raise ConsumerError("profile three-way assignment has the wrong treatment")
        for field in PAIR_INVARIANT_FIELDS:
            if assignment[field] != reference[field]:
                raise ConsumerError(
                    "profile three-way assignments differ outside treatment/profile"
                )
        if assignment["source"] != reference["source"]:
            raise ConsumerError(
                "profile three-way assignments do not share task source"
            )
...
def _decode_artifacts(observation: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact in observation["artifacts"]:
        ...
        try:
            ...
            raise ConsumerError(
                "observation artifact is not strict base64url"
            ) from error
        if (
            "=" in encoded
...
def _validate_reproduction(
    ...
    manifest_versions = [
        entry.get("version_id") for entry in manifest.get("entries", [])
    ]
    explained_versions = [
...
def validate_observation(
    ...
    *,
    ...
    if tuple(item["tool"] for item in value["tool_observations"]) != REQUIRED_TOOLS:
        raise ConsumerError(
            "consumer observation has an incomplete production call trace"
        )
    ...
    expected_artifacts.update(
        name for name, enabled in assignment["flows"].items() if enabled
    )
    if set(decoded) != expected_artifacts:
        raise ConsumerError(
            "consumer observation artifact set disagrees with assignment"
        )
    _validate_reproduction(value, decoded)
    return value
...
def _three_way_order(reference: dict[str, Any]) -> tuple[str, ...]:
    identity_fields = {
    ...
    return tuple(
        sorted(
            THREE_WAY_TREATMENTS,
            key=lambda treatment: hashlib.sha256(
                canonical_bytes({**identity_fields, "treatment": treatment})
            ).digest(),
        )
    )
...
def run_pair(
    *,
    ...
    treatments: dict[str, tuple[dict[str, Any], bytes, Path, str]] = {
        "champion": (
            champion,
            champion_bytes,
            champion_executable,
    ...
    order = (
        ("candidate", "champion") if _order_key(champion) else ("champion", "candidate")
    )
...
def _run_three_way(
    *,
    honey_assignment_path: Path,
    champion_assignment_path: Path,
    candidate_assignment_path: Path,
    honey_executable_path: Path,
    champion_executable_path: Path,
    candidate_executable_path: Path,
    cwd: Path,
    state: Path,
    schemas: Path,
    assignment_validator: Callable[[dict[str, dict[str, Any]]], None],
    timeout_seconds: int = 300,
) -> dict[str, Any]:
    if (
        isinstance(timeout_seconds, bool)
    ...
    registry = SchemaRegistry(schemas)
    paths = {
        "honey": honey_assignment_path,
        "champion": champion_assignment_path,
        "candidate": candidate_assignment_path,
    }
    executable_paths = {
        "honey": honey_executable_path,
        "champion": champion_executable_path,
        "candidate": candidate_executable_path,
    }
    assignments: dict[str, dict[str, Any]] = {}
    assignment_bytes: dict[str, bytes] = {}
    executables: dict[str, Path] = {}
    executable_digests: dict[str, str] = {}
    for treatment in THREE_WAY_TREATMENTS:
        assignments[treatment], assignment_bytes[treatment] = load_assignment(
            paths[treatment], registry
        )
        executables[treatment], executable_digests[treatment] = _executable(
            executable_paths[treatment]
        )
    assignment_validator(assignments)
    environment = sanitized_environment(state)
    order = _three_way_order(assignments["honey"])
    observations: dict[str, dict[str, Any]] = {}
    outcomes: dict[str, dict[str, Any]] = {}
    for treatment in order:
        try:
            stdout, _stderr = _run_process(
                executables[treatment],
                assignment_bytes[treatment],
                cwd=cwd,
                environment=environment,
                timeout_seconds=timeout_seconds,
            )
        except ConsumerProcessFailure as error:
            outcomes[treatment] = {
                "status": "failed",
                "observation_id": None,
        ...
        observations[treatment] = validate_observation(
            stdout,
            assignment=assignments[treatment],
            assignment_bytes=assignment_bytes[treatment],
            executable_digest=executable_digests[treatment],
            registry=registry,
        )
        outcomes[treatment] = {
            "status": "completed",
            "observation_id": observations[treatment]["observation_id"],
            "failure": None,
        }
    reference = assignments["honey"]
    body: dict[str, Any] = {
        "schema_version": "cigar.benchmark-three-way.v1",
        "run_id": reference["run_id"],
        "pair_id": reference["pair_id"],
        "task_id": reference["task_id"],
        "order": list(order),
        "assignment_digests": {
            treatment: multihash_bytes(assignment_bytes[treatment])
            for treatment in THREE_WAY_TREATMENTS
        },
        "consumer_digests": executable_digests,
        "outcomes": outcomes,
        "observation_ids": {
            treatment: outcomes[treatment]["observation_id"]
            for treatment in THREE_WAY_TREATMENTS
        },
        "observations": [
        ...
        "status": (
            "completed"
            if len(observations) == len(THREE_WAY_TREATMENTS)
            else "completed-with-treatment-failures"
        ),
    }
    result = {**body, "three_way_result_id": identity(body)}
    registry.validate("benchmark-three-way-v1.schema.json", result)
    return result
...
def run_three_way(
    *,
    ...
    """Runs one frozen balanced.v1 Honey/champion/candidate comparison."""

    return _run_three_way(
        honey_assignment_path=honey_assignment_path,
        champion_assignment_path=champion_assignment_path,
        candidate_assignment_path=candidate_assignment_path,
        honey_executable_path=honey_executable_path,
        champion_executable_path=champion_executable_path,
        candidate_executable_path=candidate_executable_path,
        cwd=cwd,
        state=state,
        schemas=schemas,
        assignment_validator=_validate_three_way,
        timeout_seconds=timeout_seconds,
    )


def run_profile_three_way(
    *,
    honey_assignment_path: Path,
    champion_assignment_path: Path,
    candidate_assignment_path: Path,
    ...
    """Runs one sequential intelligence-profile comparison with bounded failures."""
    ...
    return _run_three_way(
        ...
        state=state,
        schemas=schemas,
        assignment_validator=_validate_profile_three_way,
        timeout_seconds=timeout_seconds,
    )

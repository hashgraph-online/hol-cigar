...
from tools.refinement.codex_cli_agent import codex_cli_adapter
...
class LoopController:
    ...
    def _early_reject(
        ...
        *,
        ...
        adapter_transcript: list[dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
        message = str(error)
                ...
                if effective == "materialized":
                    ...
                    try:
                        ...
                        self._early_reject(
                            ...
                            actual=actual,
                            adapter_usage=usage,
                            adapter_transcript=list(controller.transcript),
                        )
                        continue
                    usage = outcome["usage"]
                    actual = {
...
def adapter_factory_from_profile(
    ...
    *,
    ...
    def build(_packet: dict[str, Any]) -> BaseAdapter:
        ...
        if adapter_id == "codex-cli-tools-v1":
            if codex_executable is None:
                raise LoopError("Codex CLI adapter requires --codex-executable")
            return codex_cli_adapter(
                executable=codex_executable,
                model=profile["model"],
                maximum_turns=maximum_turns,
                timeout_seconds=profile["timeout_seconds"],
                maximum_response_bytes=profile["maximum_response_bytes"],
                reasoning_effort=profile["sampling"]["reasoning_effort"],
            )

...
def _verifier_result(
    *,
    ...
    if (
        ...
        or result["passed"] != all(check["passed"] for check in result["checks"])
    ):
...
def _evidence_aliases(oracle: dict[str, Any]) -> dict[str, set[str]]:
    ...
    for evidence_id in (
        oracle["relevant_evidence"] + oracle["prohibited_evidence"]
    ):
...
def _derive_metrics(
    *,
    ...
    sufficient_claims = sum(
        all(_present(evidence_id, aliases, selected) for evidence_id in claim["evidence_ids"])
    ...
    metrics = [
        _metric("verified_task_success", int(verifier["passed"]), 1, "boolean", ["verifier-result"]),
        _metric("critical_context_recall", present_weight, critical_weight, "ratio", ["observation", "oracle"], applicable=critical_weight > 0),
        _metric("evidence_token_precision", relevant_tokens, total_evidence_tokens, "ratio", ["observation", "oracle"], applicable=total_evidence_tokens > 0),
        _metric("evidence_item_precision", len(relevant_blocks), len(evidence_blocks), "ratio", ["observation", "oracle"], applicable=bool(evidence_blocks)),
        _metric("citation_recall", len(cited_gold_claims), len(required_claims), "ratio", claims_sources, applicable=claims is not None and bool(required_claims)),
        _metric("citation_precision", citation_valid, citation_total, "ratio", claims_sources, applicable=claims is not None and citation_total > 0),
        _metric("unsupported_claim_rate", unsupported, len(claim_rows), "ratio", claims_sources, applicable=claims is not None and bool(claim_rows)),
        ...
        _metric("prohibited_materialized_tokens", prohibited_tokens, 1, "tokens", ["observation", "oracle"]),
        _metric("digest_mismatches", 0, 1, "count", ["observation"]),
        _metric("unsafe_effect_retries", observation["effect_replay"]["unsafe_retries"], 1, "count", ["observation"]),
        _metric("budget_overflow", int(sum(block["tokens"] for block in selected_blocks) > task["contract"]["token_budget"]), 1, "count", ["observation", "task"]),
        _metric("physical_input_tokens", observation["resources"]["physical_input_tokens"], 1, "tokens", ["observation"]),
        _metric("cache_read_tokens", observation["resources"]["cache_read_tokens"], 1, "tokens", ["observation"]),
        _metric("cache_write_tokens", observation["resources"]["cache_write_tokens"], 1, "tokens", ["observation"]),
        _metric("output_tokens", observation["resources"]["output_tokens"], 1, "tokens", ["observation"]),
        _metric("latency_ms", observation["resources"]["latency_ms"], 1, "milliseconds", ["observation"]),
        _metric("cpu_ms", observation["resources"]["cpu_ms"], 1, "milliseconds", ["observation"], applicable=observation["resources"]["cpu_measured"]),
        _metric("peak_rss_bytes", observation["resources"]["peak_rss_bytes"], 1, "bytes", ["observation"], applicable=observation["resources"]["peak_rss_measured"]),
        _metric("cost_usd", observation["resources"]["cost_usd"], 1, "usd", ["observation"]),
        _metric("handoffs", observation["effect_replay"]["handoffs"], 1, "count", ["observation"]),
        _metric("effects", observation["effect_replay"]["effects"], 1, "count", ["observation"]),
        _metric("replay_dispatches", observation["effect_replay"]["replay_dispatches"], 1, "count", ["observation"]),
        _metric("human_agreement", agreement_numerator, agreement_denominator, "ratio", ["adjudication"] if adjudication is not None else ["oracle"], applicable=adjudication is not None and agreement_denominator > 0),
    ]
    ...
    if sum(block["tokens"] for block in selected_blocks) > task["contract"]["token_budget"]:
        violations.append(
            {
...
def verify_attestation(
    ...
    *,
    ...
    if any(attachments.get(name) != digest for name, digest in required_bindings.items()):
    ...
    for metric in evaluation["metrics"]:
        ...
        if not metric["applicable"]:
            if any(metric[field] != 0 for field in ("numerator", "denominator", "value")):

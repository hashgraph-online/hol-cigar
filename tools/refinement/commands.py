...
def default_registry() -> CommandRegistry:
    return CommandRegistry(
        (
            ...
            CommandSpec(
                "refinement-evaluator-tests",
                (
                    sys.executable,
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r04_evaluator",
                    "-q",
                ),
                300,
            ),
            CommandSpec(
                "refinement-corpus-tests",
                (
                    sys.executable,
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r05_corpus",
                    "-q",
                ),
                300,
            ),
            CommandSpec(
                "refinement-statistics-tests",
                (
                    sys.executable,
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r06_statistics",
                    "-q",
                ),
                300,
            ),
            ...
            CommandSpec(
                "refinement-experiment-tests",
                (
                    sys.executable,
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r08_experiment",
                    "-q",
                ),
                300,
            ),
            CommandSpec(
                ...
                (
                    ...
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r09_profiles",
                    "-q",
                ),
                300,
            ),
            CommandSpec(
                "refinement-operations-tests",
                (
                    sys.executable,
                    "-m",
                    "unittest",
                    "tools.refinement.tests.test_r10_operations",
                    "-q",
                ),
                300,
            ),
...
def sanitized_environment(state: Path) -> dict[str, str]:
    ...
    uv_cache = _validated_uv_cache_directory()
    if uv_cache is None:
        environment.pop("UV_CACHE_DIR", None)
    else:
        environment["UV_CACHE_DIR"] = str(uv_cache)
    cargo = shutil.which("cargo", path=environment.get("PATH"))
    if cargo is not None and "CARGO_HOME" not in environment:
        cargo_home = Path(cargo).absolute().parent.parent
        metadata = cargo_home.stat(follow_symlinks=False)
        if (
            stat.S_ISDIR(metadata.st_mode)
            and metadata.st_uid == os.geteuid()
            and not cargo_home.is_symlink()
            and cargo_home.resolve(strict=True) == cargo_home
            and not stat.S_IMODE(metadata.st_mode) & 0o022
        ):
            environment["CARGO_HOME"] = str(cargo_home)
    if "RUSTUP_HOME" not in environment and "CARGO_HOME" in environment:
        rustup_home = Path(environment["CARGO_HOME"]).parent / ".rustup"
        if rustup_home.exists():
            metadata = rustup_home.stat(follow_symlinks=False)
            if (
                stat.S_ISDIR(metadata.st_mode)
                and metadata.st_uid == os.geteuid()
                and not rustup_home.is_symlink()
                and rustup_home.resolve(strict=True) == rustup_home
                and not stat.S_IMODE(metadata.st_mode) & 0o022
            ):
...
def _validated_uv_cache_directory() -> Path | None:
    ...
    if configured:
        ...
        if sys.platform == "darwin":
            ...
            candidates = [Path(local) / "uv" / "cache"] if local else []
        else:
            base = os.environ.get("XDG_CACHE_HOME")
            candidates = [Path(base) / "uv"] if base else [home / ".cache" / "uv"]
...
def _resolved_executable(
    argv0: str, environment: dict[str, str]
) -> tuple[str, str, str]:
    selected = (
    ...
    if not invocation.is_file():
        raise CommandError("named command invocation path is unsafe")
    resolved, digest = _digest_executable(invocation)
    return str(invocation), resolved, digest
...
def run_named(
    ...
    *,
    ...
    (
        ...
        executable_path,
        executable_sha256,
    ) = _resolved_executable(spec.argv[0], environment)
    (
        launcher_python_invocation_path,
        launcher_python,
        launcher_python_sha256,
    ) = _resolved_executable(sys.executable, environment)
    ...
    result: dict[str, Any] = {
        ...
        "executable_invocation_path": executable_invocation_path,
        "executable_path": executable_path,
        "executable_sha256": executable_sha256,
        "launcher_python_invocation_path": launcher_python_invocation_path,

...
def main() -> int:
    if len(sys.argv) < 5 or len(sys.argv) > MAXIMUM_ARGUMENTS + 4:
        return _fail()
    try:
    ...
    if (
        ...
        or invocation.resolve(strict=True) != executable
    ):
        return _fail()
    ...
    os.execve(invocation, [str(invocation), *arguments], os.environ)
    return _fail()

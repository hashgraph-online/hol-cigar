#!/usr/bin/env bash
# Creates one short-lived certificate identity shared by both physical-replication nodes.
set -Eeuo pipefail

umask 077

readonly TLS_DIRECTORY="/var/lib/postgresql/cigar-failover-tls"
...
if [[ ! -s "$CA_CERTIFICATE" || ! -s "$SERVER_CERTIFICATE" || ! -s "$SERVER_PRIVATE_KEY" ]]; then
  ...
  openssl req -x509 -new -sha256 \
    ...
    -out "$CA_CERTIFICATE" \
    -days 2 \
    -subj "/CN=CIGAR Failover Qualification CA" >/dev/null 2>&1
  openssl genrsa -out "$SERVER_PRIVATE_KEY" 3072 >/dev/null 2>&1
  openssl req -new -sha256 \
    -key "$SERVER_PRIVATE_KEY" \
    -out "$TLS_DIRECTORY/server.csr" \
    -subj "/CN=localhost" \
    -addext "subjectAltName=DNS:localhost,DNS:primary,DNS:standby,DNS:router,IP:127.0.0.1" \
...
# Preserve the upstream PostgreSQL 18 entrypoint's directory-creation assumptions.
umask 022

...
This is the unpublished, unsupported development package for CIGAR Honey `0.9.4`, targeting Claude Code `2.1.207` on Apple silicon macOS. The declared version range and platforms define a future qualification scope only; they are not evidence of installed compatibility, signing, release qualification, publication, or support. The Honey installer rejects every version or host outside that narrow target. Portable Linux and Windows runtime/test assets remain non-installable until their distributed artifacts pass separate native-platform qualification. Any Honey exercise also requires matching `cigar`, `cigar-mcp`, and `cigar-claude-hook` executables from one verified native archive. The installer captures the exact hook and MCP bytes and stages them under `${CLAUDE_PLUGIN_ROOT}/bin`; neither runtime command is resolved through ambient `PATH`.
...
CIGAR_EVIDENCE_DIR=/private/tmp/cigar-claude-plugin-build \
  python3 scripts/release/build_claude_code_plugin.py \
    --runtime-archive /private/tmp/cigar-native-build/cigar-0.9.4-aarch64-apple-darwin.tar.gz
```
...
CIGAR_EVIDENCE_DIR=/private/tmp/cigar-claude-installed-qualification \
  python3 scripts/release/qualify_claude_code_plugin.py \
    ...
    --plugin-archive /private/tmp/cigar-claude-plugin-build/cigar-claude-code-0.9.4.tar.gz \

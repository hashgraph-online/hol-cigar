...
#[cfg(feature = "experimental-profiles")]
use cigar_retrieval::{QueryPlannerProfile, RetrievalProfile};
...
#[cfg(feature = "experimental-profiles")]
#[derive(Serialize)]
struct RequirementAwarePlannerProfilePin {
    schema_version: &'static str,
    base: ExperimentalPlannerProfilePin,
    critical_requirement_gain: i64,
    requirement_gain: i64,
    concept_gain: i64,
    source_diversity_gain: i64,
    section_diversity_gain: i64,
    kind_diversity_gain: i64,
    generic_match_penalty: i64,
    redundant_requirement_penalty: i64,
    redundant_concept_penalty: i64,
}
...
pub async fn run(
    ...
    let requested_profile = assignment
        .intelligence_profile
        .unwrap_or(IntelligenceProfile::BalancedV1);
    #[cfg(not(feature = "experimental-profiles"))]
    if requested_profile != IntelligenceProfile::BalancedV1 {
        return Err(ConsumerError::new("unsupported_intelligence_profile"));
    }
    #[cfg(feature = "experimental-profiles")]
    let (retrieval_profile, compiler_profile) = match requested_profile {
        ...
        IntelligenceProfile::BalancedV2Candidate2 => (
        ...
        IntelligenceProfile::BalancedV4 => {
            (RetrievalProfile::BalancedV4, CompilerProfile::balanced_v4())
        }
    };
    #[cfg(not(feature = "experimental-profiles"))]
    let compiler_profile = CompilerProfile::default();
    #[cfg(feature = "experimental-profiles")]
    ...
    let application = CatalogContextApplication::new(
        Arc::clone(&store),
        identities,
        authorizer,
        retriever.clone(),
    ...
    let application = application
        ...
        .with_benchmark_query_planner_profile(match retrieval_profile {
            ...
            RetrievalProfile::BalancedV2RequirementAwareCandidate => {
                QueryPlannerProfile::balanced_v2_requirement_aware_candidate()
            }
            RetrievalProfile::BalancedV4 => QueryPlannerProfile::balanced_v4(),
        });
        ...
        .build_generation(
            IndexBuild {
                ...
                configuration_digest: {
                    #[cfg(feature = "experimental-profiles")]
                    {
                        match retrieval_profile {
                            RetrievalProfile::BalancedV1 => {
                                multihash(b"cigar.benchmark-index.lexical.v1")?
                            }
                            RetrievalProfile::BalancedV2Candidate
                            | RetrievalProfile::BalancedV2RequirementAwareCandidate
                            | RetrievalProfile::BalancedV4 => retrieval_profile
                                .digest()
    ...
    let planner = match retrieval_profile {
        ...
        profile @ (RetrievalProfile::BalancedV2RequirementAwareCandidate
        | RetrievalProfile::BalancedV4) => {
            let selection = if profile == RetrievalProfile::BalancedV4 {
                QueryPlannerProfile::balanced_v4().candidate_selection
            } else {
                QueryPlannerProfile::balanced_v2_requirement_aware_candidate().candidate_selection
            };
            multihash(&canonical_json(&RequirementAwarePlannerProfilePin {
                schema_version: if profile == RetrievalProfile::BalancedV4 {
                    "cigar.query-planner-profile.balanced.v4"
                } else {
                    "cigar.query-planner-profile.v2-candidate.2"
                },
                base: ExperimentalPlannerProfilePin {
                    schema_version: "cigar.query-planner-profile.v2-candidate.1",
                    base: base_planner,
                ...
                critical_requirement_gain: selection.critical_requirement_gain,
                requirement_gain: selection.requirement_gain,
                concept_gain: selection.concept_gain,
                source_diversity_gain: selection.source_diversity_gain,
                section_diversity_gain: selection.section_diversity_gain,
                kind_diversity_gain: selection.kind_diversity_gain,
                generic_match_penalty: selection.generic_match_penalty,
                redundant_requirement_penalty: selection.redundant_requirement_penalty,
                redundant_concept_penalty: selection.redundant_concept_penalty,
            })?)?
        }
    };
    #[cfg(not(feature = "experimental-profiles"))]
    let planner = multihash(&canonical_json(&base_planner)?)?;
...
fn verify_exact_manifest_entries(
    ...
    for entry in &manifest.entries {
        let request = RetrievalRequest {
            ...
            #[cfg(not(feature = "honey-0-9-1-compat"))]

...
1. Run `scripts/release/beta_profile.py check` and the beta runtime/release test suites against one clean detached candidate. Then run `scripts/release/beta_artifacts.py freeze-source --git /absolute/path/to/git --out /absolute/external/source-freeze`; the host-independent freeze requires a clean, stable Git commit/tree and writes exactly the deterministic source archive plus its canonical source descriptor outside the repository.
...
3. On the approved Ubuntu 24.04 x86-64/glibc 2.39 builder, with locked dependencies and independently enforced no-egress, run `scripts/release/beta_artifacts.py build --source-freeze /absolute/external/source-freeze`. The build rejects a checkout whose clean commit/tree differs from the freeze, materializes build input only from the verified archive, copies the frozen archive and descriptor bytes unchanged, and produces an unsigned owner-only candidate containing the closed six-archive inventory plus checksums, CycloneDX, SPDX, provenance, manifest, and build-verification receipt.
...
6. Run `scripts/release/beta_release.py plan` to create the canonical release-evidence document. Send only those exact canonical bytes to the approved isolated signer; no production private key may enter the workspace.
7. Run `scripts/release/beta_release.py assemble` with the returned detached release-evidence signature, then run `scripts/release/beta_release.py verify` from a clean offline environment using the independently distributed trust policy.

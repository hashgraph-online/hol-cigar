...
use crate::{
    ...
    RetrievalErrorCode, StagedRetrievalResult,
};
use cigar_protocol::{
        ...
        for (planned, executed) in plan.stages.iter().zip(&retrieval.stages) {
            ...
            for candidate in &executed.batch.candidates {
                ...
                match merged.get_mut(&candidate.version_id) {
                    Some(current) => {
                        merge_same_version(&mut current.candidate, candidate)?;
                        current
                            .requirement_indices
...
fn validate_candidate(candidate: &CandidateRef) -> Result<(), RetrievalError> {
    if candidate.features.estimated_tokens == 0
        || candidate.total_score != candidate.features.balanced_score()?
...
fn merge_same_version(
    ...
    current.total_score = current.features.balanced_score()?;
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::RequirementAwareCandidateReducer;
    use crate::{
        ...
        RetrievalContext, RetrievalDisclosure, RetrievalStage, StagedRetrievalResult,
    };
    use cigar_protocol::{
    ...
    fn candidate(
        ...
        Ok(CandidateRef {
            ...
            features,
            total_score: features.balanced_score()?,

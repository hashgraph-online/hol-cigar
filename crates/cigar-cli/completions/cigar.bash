_cigar_complete() {
    ...
    local globals="--output --deadline --config --target --embedded --local --remote --endpoint --authorization-file --input --idempotency-key --expected-revision --page-cursor --page-size --dry-run --yes --non-interactive --quiet --color --unicode --width --explain-config --security --deep"
    local words=""
    case "${COMP_WORDS[1]}" in
        ...
        *) words="init source ingest status context project focus space handoff effect replay policy backup gc diagnostics doctor serve mcp plugin release completion man help version" ;;
    esac
    case "$previous" in

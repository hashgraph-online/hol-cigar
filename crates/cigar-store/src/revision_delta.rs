...
/// Release-default delta suffix selected by the frozen storage/latency matrix.
///
/// The 256-delta protocol maximum remains accepted so an already-authenticated
/// v5 authority can still be inspected, while new Honey 0.9.2 targets use the
/// smaller suffix to bound serial compilation latency.
pub const DEFAULT_DELTAS_SINCE_CHECKPOINT_V5: usize = 4;
...
const _: () = {
    ...
    assert!(DEFAULT_DELTAS_SINCE_CHECKPOINT_V5 > 0);
    assert!(DEFAULT_DELTAS_SINCE_CHECKPOINT_V5 <= MAX_DELTAS_SINCE_CHECKPOINT_V5);

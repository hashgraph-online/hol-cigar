...
use crate::revision_delta::{
    DEFAULT_DELTAS_SINCE_CHECKPOINT_V5, MAX_ACCUMULATED_DELTA_BYTES_V5,
    MAX_DELTAS_SINCE_CHECKPOINT_V5, MAX_REPLAY_OPERATIONS_V5, MAX_REPOSITORY_CHECKPOINT_BYTES_V5,
    MAX_REPOSITORY_DELTA_BYTES_V5, MAX_REPOSITORY_DELTA_OPERATIONS_V5, PreparedRepositoryDeltaV5,
    RepositoryChainLinkV5, RepositoryCheckpointReasonV5, RepositoryCheckpointV5,
    RepositoryLogicalTotalsV5, apply_repository_delta_v5,
    catalog_mutation_commitment_from_records_v5, decode_catalog_free_state_v5,
    encode_catalog_free_state_v5, migration_receipt_schema_digest_v1, repository_chain_head_v5,
    repository_delta_from_service_v5, repository_delta_from_staged_v5,
...
const COMPRESSED_CHECKPOINT_MAGIC_V5: &[u8; 8] = b"CIGV5Z01";
const COMPRESSED_CHECKPOINT_HEADER_BYTES_V5: usize = 16;
const MAX_ENCODED_CHECKPOINT_BYTES_V5: usize =
    MAX_REPOSITORY_CHECKPOINT_BYTES_V5.saturating_add(4_096);
...
impl RepositoryPolicyV5 {
    fn qualification(capacity_profile: &str) -> Result<Self, StoreError> {
        Self::new(
            u64::try_from(DEFAULT_DELTAS_SINCE_CHECKPOINT_V5).map_err(|_error| limit_exceeded())?,
...
/// Encodes the authenticated canonical checkpoint into its SQLite storage envelope.
///
/// The chain and checkpoint digests continue to bind the uncompressed canonical
/// representation. Compression is only a deterministic physical-storage detail,
/// and the decoder retains support for the uncompressed v5 rows created by an
/// interrupted or pre-release migration.
fn encode_stored_checkpoint_v5(checkpoint: &RepositoryCheckpointV5) -> Result<Vec<u8>, StoreError> {
    let canonical = checkpoint.encode()?;
    let compressed = zstd::bulk::compress(&canonical, 3)
        .map_err(|_error| StoreError::new(StoreErrorCode::Unavailable))?;
    let stored_bytes = COMPRESSED_CHECKPOINT_HEADER_BYTES_V5
        .checked_add(compressed.len())
        .ok_or_else(limit_exceeded)?;
    if stored_bytes >= canonical.len() {
        return Ok(canonical);
    }
    let mut stored = Vec::with_capacity(stored_bytes);
    stored.extend_from_slice(COMPRESSED_CHECKPOINT_MAGIC_V5);
...
fn decode_stored_checkpoint_v5(bytes: &[u8]) -> Result<RepositoryCheckpointV5, StoreError> {
    if !bytes.starts_with(COMPRESSED_CHECKPOINT_MAGIC_V5) {
        return RepositoryCheckpointV5::decode(bytes);
    }
    if bytes.len() <= COMPRESSED_CHECKPOINT_HEADER_BYTES_V5 {
        return Err(invalid_record());
    }
    let length_bytes = bytes
        .get(COMPRESSED_CHECKPOINT_MAGIC_V5.len()..COMPRESSED_CHECKPOINT_HEADER_BYTES_V5)
        .ok_or_else(invalid_record)?;
    let expected = usize::try_from(u64::from_be_bytes(
        length_bytes.try_into().map_err(|_error| invalid_record())?,
    ))
    .map_err(|_error| limit_exceeded())?;
    if expected == 0 || expected > MAX_ENCODED_CHECKPOINT_BYTES_V5 {
        return Err(limit_exceeded());
    }
    let compressed = bytes
        .get(COMPRESSED_CHECKPOINT_HEADER_BYTES_V5..)
        .ok_or_else(invalid_record)?;
    let canonical =
        zstd::bulk::decompress(compressed, expected).map_err(|_error| invalid_record())?;
    if canonical.len() != expected {
        return Err(invalid_record());
    }
    RepositoryCheckpointV5::decode(&canonical)
}

fn live_blob_roots_v5(state: &CommittedState) -> BTreeMap<String, BTreeSet<ContentDigest>> {
    state
        .tenants
        .iter()
        .map(|(tenant, state)| {
...
fn insert_checkpoint(
    ...
    let encoded = encode_stored_checkpoint_v5(checkpoint)?;
...
pub(crate) fn bounded_startup_verification_v5(
    ...
    startup_verification_from_authenticated_v5(connection, &authenticated)
}

fn startup_verification_from_authenticated_v5(
    connection: &Connection,
    authenticated: &AuthenticatedLatestV5,
) -> Result<SqliteStartupVerificationV5, StoreError> {
    ...
    Ok(SqliteStartupVerificationV5 {
        ...
        chain_head: authenticated.authority.chain_head.clone(),
        policy_digest: authenticated.authority.policy_digest.clone(),
    })
}

fn recover_bounded_startup_with_state_v5(
    connection: &mut Connection,
) -> Result<(SqliteStartupVerificationV5, CommittedState), StoreError> {
    let authenticated = authenticate_latest_repository_state_v5(connection)?;
    ...
    let verification = startup_verification_from_authenticated_v5(connection, &authenticated)?;
    Ok((verification, authenticated.state))
}

pub(crate) fn recover_bounded_startup_v5(
    connection: &mut Connection,
) -> Result<SqliteStartupVerificationV5, StoreError> {
    recover_bounded_startup_with_state_v5(connection).map(|(verification, _state)| verification)
}
...
pub(crate) fn retention_statistics_v5(
    ...
    retention_statistics_from_authority_v5(connection, authority)
}

/// Computes retention capacity from the authority already authenticated by the
/// enclosing atomic commit. This deliberately avoids replaying the latest
/// checkpoint suffix after every ordinary mutation.
fn retention_statistics_from_authority_v5(
    connection: &Connection,
    authority: AuthorityV5,
) -> Result<SqliteRetentionStatisticsV5, StoreError> {
...
fn enforce_retention_capacity_v5(connection: &Connection) -> Result<(), StoreError> {
    let authority = load_authority(connection)?;
    if retention_statistics_from_authority_v5(connection, authority)?.capacity_blocked {
        Err(limit_exceeded())
    } else {
        Ok(())
    }
...
fn recover_revision_anchor_v5(
    ...
    let _authenticated =
        reconstruct_repository_revision_v5(connection, authority.current_revision)?;
    recover_revision_anchor_for_revision_v5(connection, revision_anchor, authority.current_revision)
}

fn recover_revision_anchor_for_revision_v5(
    connection: &Connection,
    revision_anchor: &Path,
    current_revision: StoreRevision,
) -> Result<StoreRevision, StoreError> {
    ...
    match read_revision_anchor(revision_anchor)? {
        Some(anchor) if anchor.0 > current_revision.0 => return Err(invalid_record()),
        Some(anchor) if anchor == current_revision => return Ok(anchor),
        _ => {}
    }
    write_revision_anchor(revision_anchor, current_revision)?;
    Ok(current_revision)
}
...
fn reconstruct_repository_revision_v5(
    ...
    let checkpoint_row = connection
        ...
        .map_err(unavailable)?
        .ok_or_else(invalid_record)?;
    let checkpoint = decode_stored_checkpoint_v5(&checkpoint_row.0)?;
...
pub struct SqliteV5Store {
    ...
    latest_state: Mutex<LatestStateCacheV5>,
...
impl SqliteV5Store {
    ...
    fn open_internal(
        ...
        let latest_authority = load_authority(&connection)?;
        if latest_authority.current_revision != latest_state.revision {
            return Err(invalid_record());
        }
        let live_blob_roots = live_blob_roots_v5(&latest_state);
        verify_secure_sqlite_path(path, secure_identity)?;
        let store = Self {
            path: path.to_path_buf(),
            connection: Mutex::new(connection),
            latest_state: Mutex::new(LatestStateCacheV5 {
                authority: latest_authority,
                state: Arc::new(latest_state),
            }),
            blob_repository,
        ...
        measure_startup_stage(observer, RepositoryStartupStage::BlobReconciliation, || {
            store.blob_repository.reconcile(&live_blob_roots)
        })?;
        Ok(store)
    }
    ...
    pub fn effect_store_is_empty(&self) -> Result<bool, StoreError> {
        let connection = self.lock()?;
        let state = self.authenticated_latest_state(&connection)?;
        Ok(state
            .tenants
            .values()
    ...
    pub fn reconcile_blob_roots(&self) -> Result<(), StoreError> {
        let connection = self.lock()?;
        let state = self.authenticated_latest_state(&connection)?;
        let live = live_blob_roots_v5(&state);
    ...
    /// Returns the in-process latest state after checking its revision against
    /// the authenticated database authority. An external writer therefore
    /// invalidates the cache before it can influence a read or mutation.
    fn authenticated_latest_state(
        &self,
        connection: &Connection,
    ) -> Result<Arc<CommittedState>, StoreError> {
        let authority = load_authority(connection)?;
        let mut cached = self
            .latest_state
            .lock()
        ...
        let reconstructed = Arc::new(reconstruct_repository_snapshot_v5(
            connection,
            SnapshotSelection::Revision(authority.current_revision),
        )?);
        // A read-only transaction can hold an older SQLite snapshot while an in-process
        // writer has already advanced the shared cache. Return the transaction-consistent
        // state without ever moving the process-wide latest cache backwards.
        if reconstructed.revision >= cached.state.revision {
            *cached = LatestStateCacheV5 {
                authority,
                state: Arc::clone(&reconstructed),
            };
        }
        Ok(reconstructed)
    }

    fn publish_latest_state(
        &self,
        connection: &Connection,
        state: CommittedState,
    ) -> Result<(), StoreError> {
        let authority = load_authority(connection)?;
        if authority.current_revision != state.revision {
            return Err(StoreError::new(StoreErrorCode::RevisionConflict));
        }
        let mut cached = self
            .latest_state
            .lock()
            .map_err(|_error| StoreError::new(StoreErrorCode::Unavailable))?;
        if state.revision < cached.state.revision {
            return Err(StoreError::new(StoreErrorCode::RevisionConflict));
        }
        *cached = LatestStateCacheV5 {
            authority,
...
impl WriteTransaction for SqliteV5WriteTransaction<'_> {
    ...
    fn commit(self, idempotency: Option<IdempotencyIdentity>) -> Result<CommitReceipt, StoreError> {
        ...
        let latest = self.store.authenticated_latest_state(&connection)?;
        ...
        let (committed, state) = commit_then_publish_repository_delta_v5(
            &mut connection,
        ...
        self.store.publish_latest_state(&connection, state)?;
        let after = v5_commit_footprint(&connection, &self.store.path);
        drop(connection);
        self.store.observe_commit(RepositoryCommitMetrics {
            kind: RepositoryCommitKind::Repository,
...
impl Repository for SqliteV5Store {
    ...
    fn begin_read(
        ...
        let state = match selection {
            SnapshotSelection::Latest => self.authenticated_latest_state(&connection)?,
            SnapshotSelection::Revision(_) => {
                Arc::new(reconstruct_repository_snapshot_v5(&connection, selection)?)
            }
        };
...
impl ServiceRepository for SqliteV5Store {
    fn service_get(
        ...
        let state = self
            .authenticated_latest_state(&connection)
    ...
    fn service_list(
        ...
        let connection = self.lock().map_err(map_store_error)?;
        let state = match selection {
            SnapshotSelection::Latest => self
                .authenticated_latest_state(&connection)
                .map_err(map_store_error)?,
            SnapshotSelection::Revision(_) => Arc::new(
                reconstruct_repository_snapshot_v5(&connection, selection)
                    .map_err(map_store_error)?,
            ),
        };
    ...
    fn service_commit(
        ...
        self.publish_latest_state(&connection, committed_state)
            .map_err(map_store_error)?;
        let after = v5_commit_footprint(&connection, &self.path);
        drop(connection);
        self.observe_commit(RepositoryCommitMetrics {
...
mod tests {
    ...
    use crate::{
        ...
        ReadTransaction, RepositoryBlobStore, ServiceBatch, ServiceExpectedVersion,
        ServiceRecordLocator, ServiceRecordSelection, ServiceRecordWrite, ServiceRepository,
        ServiceResponse, SqliteStore, StoreErrorCode, WorkerLocator, WorkerUpdate,
    };
    use cigar_crypto::{
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let checkpoint_interval = policy.maximum_deltas_since_checkpoint + 1;
        let expected_checkpoint = StoreRevision((300 / checkpoint_interval) * checkpoint_interval);
        let expected_replayed = 300 - expected_checkpoint.0;
        ...
        assert_eq!(recovered.checkpoint_revision, expected_checkpoint);
        assert_eq!(recovered.replayed_deltas, expected_replayed);
        ...
        assert_eq!(bounded.checkpoint_revision, expected_checkpoint);
        assert_eq!(bounded.replayed_deltas, expected_replayed);
        assert_eq!(
    ...
    #[test]
    #[ignore = "explicit fail-fast checkpoint cadence selection matrix"]
    fn checkpoint_cadence_matrix_covers_frozen_4_delta_release_bound()
    -> Result<(), Box<dyn std::error::Error>> {
        const MUTATIONS: u64 = 512;
        let maximum_delta_bytes = u64::try_from(MAX_ACCUMULATED_DELTA_BYTES_V5)?;
        let mut observations = Vec::new();
        for cadence in [4_u64, 16, 64, 128, 256] {
            let policy = RepositoryPolicyV5::new(cadence, maximum_delta_bytes, "standard")?;
            let (_directory, mut connection) =
                target_with_receipt_deltas_and_policy(MUTATIONS, policy)?;
            let statistics = retention_statistics_v5(&connection)?;
            let started = std::time::Instant::now();
            let readiness = recover_bounded_startup_v5(&mut connection)?;
            let startup_nanoseconds = u64::try_from(started.elapsed().as_nanos())?;
            assert!(startup_nanoseconds < 30_000_000_000);
            assert!(readiness.replayed_deltas <= cadence);
            eprintln!(
                "checkpoint-cadence cadence={cadence} mutations={MUTATIONS} checkpoints={} checkpoint_bytes={} suffix_deltas={} suffix_bytes={} startup_nanoseconds={startup_nanoseconds}",
        ...
        assert!(
            observations
                .windows(2)
                .all(|pair| matches!(pair, [left, right] if left.1 > right.1))
        );
        assert!(
            observations
                .windows(2)
                .all(|pair| matches!(pair, [left, right] if left.2 > right.2))
        );
        assert!(observations.iter().any(|observation| {
            observation.0 == u64::try_from(DEFAULT_DELTAS_SINCE_CHECKPOINT_V5).unwrap_or(0)
        }));
        Ok(())
    }

    #[test]
    fn qualification_policy_uses_latency_bounded_release_checkpoint_cadence()
    -> Result<(), Box<dyn std::error::Error>> {
        let policy = RepositoryPolicyV5::qualification("standard")?;
        assert_eq!(
            policy.maximum_deltas_since_checkpoint,
            u64::try_from(DEFAULT_DELTAS_SINCE_CHECKPOINT_V5)?
        );
        assert!(
            policy.maximum_deltas_since_checkpoint < u64::try_from(MAX_DELTAS_SINCE_CHECKPOINT_V5)?
        );
        assert_eq!(policy.minimum_reconstructable_revisions, 256);
        assert_eq!(policy.minimum_verified_replay_revisions, 256);
        Ok(())
    }

    #[test]
    fn compressed_checkpoint_storage_is_bounded_authenticated_and_strict()
    -> Result<(), Box<dyn std::error::Error>> {
        let policy = RepositoryPolicyV5::qualification("standard")?;
        let (_directory, connection) = target_with_receipt_deltas_and_policy(20, policy)?;
        let stored: Vec<u8> = connection.query_row(
            "SELECT canonical_state FROM repository_checkpoints_v5
             WHERE reason = 'delta_count' ORDER BY revision DESC LIMIT 1",
            [],
            |row| row.get(0),
        )?;
        assert!(stored.starts_with(COMPRESSED_CHECKPOINT_MAGIC_V5));
        let checkpoint = decode_stored_checkpoint_v5(&stored)?;
        assert_eq!(checkpoint.revision(), StoreRevision(20));
        let canonical = checkpoint.encode()?;
        assert!(stored.len() < canonical.len());
        assert_eq!(RepositoryCheckpointV5::decode(&canonical)?, checkpoint);

        let mut wrong_length = stored.clone();
        let length_range =
            COMPRESSED_CHECKPOINT_MAGIC_V5.len()..COMPRESSED_CHECKPOINT_HEADER_BYTES_V5;
        let declared = u64::from_be_bytes(
            wrong_length
                .get(length_range.clone())
                .ok_or("missing length")?
                .try_into()?,
        );
        wrong_length
            .get_mut(length_range)
            .ok_or("missing length")?
            .copy_from_slice(
                &declared
                    .checked_add(1)
        ...
        assert!(decode_stored_checkpoint_v5(&wrong_length).is_err());

        let mut trailing = stored;
        trailing.push(0);
        assert!(decode_stored_checkpoint_v5(&trailing).is_err());
        Ok(())
    }
    ...
    #[test]
    fn latest_state_cache_revalidates_external_handle_revisions()
    -> Result<(), Box<dyn std::error::Error>> {
        let directory = tempfile::tempdir()?;
        let path = directory.path().join("cache-revalidation-v5.sqlite3");
        let provider = Arc::new(MemoryKeyProvider::default());
        let tenant = tenant()?;
        let key = provider.create(CreateKeyRequest {
            tenant: tenant.as_str().to_owned(),
        ...
        let local = LocalBlobStore::open(directory.path().join("cache-blobs"), provider)?;
        let blobs: Arc<dyn RepositoryBlobStore> =
            Arc::new(LocalRepositoryBlobStore::new(local, key.key_ref, 1));
        let first = open_production_v5_store(path.clone(), Arc::clone(&blobs))?;
        let second = SqliteV5Store::open_with_blob_repository_and_capacity_profile(
            &path,
            blobs,
            SqliteCapacityProfile::Standard,
        )?;
        let worker = WorkerLocator::new(tenant, "cache-revalidation-worker")?;
        let claimed = first.worker_update(
            &worker,
            WorkerUpdate::Claim {
                expected: ServiceExpectedVersion::Absent,
                owner: "cache-test".to_owned(),
                now_unix_nanos: 1,
                expires_at_unix_nanos: 100,
            },
            &CancellationToken::default(),
        ...
        let first_cached = {
            let connection = first.lock()?;
            first.authenticated_latest_state(&connection)?
        };
        let first_cached_again = {
            let connection = first.lock()?;
            first.authenticated_latest_state(&connection)?
        };
        assert!(Arc::ptr_eq(&first_cached, &first_cached_again));
        assert_eq!(
            second
                .begin_read(
                    AccessContext::new(worker.tenant_id().clone(), "test")?,
                    SnapshotSelection::Latest,
                    CancellationToken::default(),
                )?
                .revision(),
            claimed.store_revision()
        );
        assert_eq!(
            second
                .worker_get(&worker, &CancellationToken::default())?
                .ok_or("second handle missed first-handle revision")?,
            claimed
        );
        let checkpointed = second.worker_update(
            &worker,
            WorkerUpdate::Checkpoint {
                expected: ServiceExpectedVersion::Version(claimed.version()),
                owner: "cache-test".to_owned(),
                fencing_token: claimed.fencing_token(),
                cursor: b"checkpoint".to_vec(),
        ...
        assert_eq!(
            first
                .worker_get(&worker, &CancellationToken::default())?
                .ok_or("first handle missed second-handle revision")?,
            checkpointed
        );
        assert_eq!(
            first
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let atom_template = fixture.atoms.first().ok_or("missing fixture atom")?;
        let policy = RepositoryPolicyV5::qualification("standard")?;
        let maximum_expected_checkpoints = MUTATIONS
            .div_ceil(policy.maximum_deltas_since_checkpoint + 1)
            .checked_add(1)
        ...
        assert!(
            counts.2 <= i64::try_from(maximum_expected_checkpoints)?,
            "retained checkpoint count must match the frozen cadence bound"
        );

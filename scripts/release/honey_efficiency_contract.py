...
FIXTURE_SHA256 = "0a3637e53a6ba17f1662460434cb0406478656f392df50907fd002bd6d7a5c82"
...
EXPECTED_GATE_THRESHOLDS: dict[str, tuple[tuple[str, str, bool | int, str], ...]] = {
    ...
    "H91-G004": (("physical_growth_bytes_per_compilation", "lt", 1_048_576, "bytes"),),
    "H91-G005": (
        (
            "serial_latency_slope_ns_per_request",
            "le",
            10_000_000,
            "nanoseconds_per_request",
        ),
    ...
    "H91-G010": (
        ("duplicate_selected_percent_millionths", "le", 50_000, "percent_millionths"),
    ),
    "H91-G011": (
    ...
    "H91-G012": (
        (
            "budget_displaced_selected_ratio_millionths",
            "lt",
            10_000_000,
            "ratio_millionths",
        ),
    ...
    "H91-G014": (
        ("required_source_coverage_millionths", "eq", 1_000_000, "millionths"),
    ),
    "H91-G015": tuple(
...
def validate_fixture_manifest(document: Any, payload: bytes) -> Mapping[str, Any]:
    ...
    expected_retention = {
        ...
        "maximum_deltas_since_checkpoint": 4,
...
def validate_verified_copy_descriptor(
    ...
    if root["status"] == "unbound":
        ...
        _integer(
            binding["source_revision"], 1, 18_446_744_073_709_551_615, "source revision"
        )
    else:
...
def validate_qualification_profile(document: Any, payload: bytes) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise EfficiencyContractError(
            "qualification profile identity or claims drifted"
        )
    ...
    paired = _keys(inputs[5], {"id", "external", "sha256"}, "paired benchmark input")
    ...
    for row in gates:
        ...
        if (
            not isinstance(row["criterion"], str)
            or not 1 <= len(row["criterion"]) <= 1024
        ):
...
def _validate_scalar(value: Any, label: str) -> bool | int:
    if isinstance(value, bool):
        return value
    return _integer(
        value, -9_223_372_036_854_775_808, 18_446_744_073_709_551_615, label
    )


def _threshold_passed(
    operator: str, expected: bool | int, observed: bool | int
) -> bool:
...
def validate_report(
    ...
    fixtures = _keys(
        root["fixtures"], {"manifest_sha256", "entries"}, "report fixtures"
    )
    ...
    for entry in entries:
        ...
        if row["kind"] == "generated":
            ...
            if expected_fixture_digests.get(identifier) != row["sha256"]:
                raise EfficiencyContractError(
                    "report generated fixture binding drifted"
                )
    ...
    for field, expected in (
        ...
        if environment[field] != expected:
            raise EfficiencyContractError(
                "report environment violates frozen conditions"
            )
    for field in ("os_version", "kernel"):
        if (
            not isinstance(environment[field], str)
            or not 1 <= len(environment[field]) <= 128
        ):
    ...
    for stage in stage_metrics:
        ...
        if identifier in stage_ids or row["unit"] not in {
            "bytes",
            "count",
            "nanoseconds",
        }:
    ...
    for index, gate in enumerate(gate_results):
        ...
        if (
            ...
            or row["release_gate_id"] != expected_release_gates[gate_id]
        ):
            raise EfficiencyContractError(
                "gate result inventory or release binding drifted"
            )
        ...
        for measurement in _sequence(row["measurements"], 1, 64, "gate measurements"):
            ...
            if name in measurements or item["unit"] not in MEASUREMENT_UNITS:
                raise EfficiencyContractError(
                    "gate measurement identity or unit is invalid"
                )
            measurements[name] = (
                _validate_scalar(item["value"], "measurement"),
                item["unit"],
            )
        passed = True
        ...
        for threshold in thresholds:
            ...
            if (
                ...
                raise EfficiencyContractError(
                    "gate threshold cannot be evaluated exactly"
                )
        ...
        if row["status"] != expected_status:
            raise EfficiencyContractError(
                "gate status disagrees with measured thresholds"
            )
...
def main() -> int:
    ...
    try:
        ...
        if arguments.command == "validate-report":
            ...
            validate_raw_attachment(
                validated, arguments.raw_observations.resolve(strict=True)
            )
        print(
            json.dumps(
                {"status": "pass", **bindings}, sort_keys=True, separators=(",", ":")
            )
        )
        return 0

...
_TEXT_SUFFIXES = {
    ".bash", ".c", ".cc", ".cmd", ".conf", ".cpp", ".css", ".fish", ".go", ".h", ".html",
    ".csv", ".graphql", ".ini", ".java", ".js", ".json", ".lock", ".md", ".mjs", ".mod",
    ".proto", ".ps1", ".py", ".rs", ".sh", ".sql", ".sum", ".svg", ".toml", ".ts", ".txt",
    ".xml", ".yaml", ".yml", ".zsh",
}
_TEXT_NAMES = {
    ".dockerignore", ".gitignore", ".npmignore", "Dockerfile", "LICENSE", "Makefile", "NOTICE",
    "README", "SHA256SUMS", "justfile",
}


def _validate_contract(contract: Any) -> dict[str, Any]:
    required_keys = {
        "schema_version", "id", "formats", "allow", "deny", "required", "symlinks", "line_endings",
        "modes", "max_entries", "max_member_bytes", "max_total_bytes", "content_scan",
        "content_scan_exemptions",
    }
    optional_keys = {
        "required_any", "required_patterns", "version_binding", "abi_binding", "checksum_manifest",
    ...
    limits = [contract.get(name) for name in ("max_entries", "max_member_bytes", "max_total_bytes")]
    if (
        not all(isinstance(value, int) and not isinstance(value, bool) and value > 0 for value in limits)
    ...
    if contract["id"] == "oci-image-v1":
        if not isinstance(layer_limit, int) or isinstance(layer_limit, bool) or layer_limit <= 0:
            raise ReleaseError("OCI package contract has no valid uncompressed-layer limit")
    elif layer_limit is not None:
        raise ReleaseError("non-OCI package contract declares an OCI layer limit")
    if contract.get("symlinks") != "forbid" or contract.get("line_endings") != "lf" or contract.get("content_scan") is not True:
...
def _binding_value(payloads: dict[str, bytes | None], binding: Any, label: str) -> Any:
    if not isinstance(binding, dict) or set(binding) != {"path_pattern", "format", "json_pointer"}:
        raise ReleaseError(f"invalid {label} binding")
    if binding["format"] != "json" or not isinstance(binding["path_pattern"], str) or not isinstance(binding["json_pointer"], str):
        raise ReleaseError(f"invalid {label} binding")
...
def _validate_checksum_manifest(
    ...
    if payload is None:
        raise ReleaseError("internal checksum manifest is missing or exceeds the metadata size limit")
    try:
    ...
    for number, line in enumerate(lines, 1):
        ...
        if relative in observed or relative in {manifest_path, "RELEASE-METADATA.json"}:
            raise ReleaseError(f"internal checksum manifest path is duplicate or forbidden: {relative}")
        attribute = attributes.get(relative)
        if not isinstance(attribute, dict) or attribute.get("kind") != "file":
            raise ReleaseError(f"internal checksum manifest references a missing regular file: {relative}")
        if attribute.get("sha256") != fields[0]:
            raise ReleaseError(f"internal checksum manifest digest mismatch: {relative}")
        observed[relative] = fields[0]
        observed_order.append(relative)
    expected_paths = {
        name
        for name, attribute in attributes.items()
        if attribute.get("kind") == "file" and name not in {manifest_path, "RELEASE-METADATA.json"}
    }
    ...
    if observed_order != sorted(observed_order, key=lambda value: value.encode("utf-8")):
        raise ReleaseError("internal checksum manifest paths are not in deterministic UTF-8 byte order")
...
def _inspect_payload(
    ...
    if retain and expected_size > _MAX_RETAINED_MEMBER_BYTES:
        raise ReleaseError(f"metadata-bearing archive member exceeds {_MAX_RETAINED_MEMBER_BYTES} bytes: {name}")
    ...
    return bytes(retained) if retained is not None else None, digest.hexdigest(), contains_carriage_return, sorted(findings)
    ...
    try:
        with tarfile.open(path, mode="r:*") as archive:
            for member in archive:
                ...
                if member.isdir():
                    attributes[name] = {"kind": "directory", "mode": member.mode, "mtime": member.mtime, "uid": member.uid, "gid": member.gid}
                    continue
                ...
                with handle:
                    payload, digest, contains_cr, content_findings = _inspect_payload(
                        handle, member.size, name, retained_patterns, content_scan, exemptions
                    )
                if payload is not None:
                    retained_total += len(payload)
                    if retained_total > _MAX_RETAINED_TOTAL_BYTES:
                        raise ReleaseError("archive metadata payloads exceed the bounded retention limit")
                payloads[name] = payload
                attributes[name] = {
                    "kind": "file", "mode": member.mode, "mtime": member.mtime, "uid": member.uid, "gid": member.gid,
                    "size": member.size, "sha256": digest, "contains_cr": contains_cr, "content_findings": content_findings,
                }
        ...
        with zipfile.ZipFile(path) as archive:
            for member in archive.infolist():
                ...
                if kind_bits == stat.S_IFLNK:
                    raise ReleaseError(f"archive links are forbidden: {name}")
                if member.is_dir():
                    attributes[name] = {"kind": "directory", "mode": mode or 0o755, "mtime": calendar.timegm((*member.date_time, 0, 0, 0))}
                    continue
                ...
                with archive.open(member) as handle:
                    payload, digest, contains_cr, content_findings = _inspect_payload(
                        handle, member.file_size, name, retained_patterns, content_scan, exemptions
                    )
                if payload is not None:
                    retained_total += len(payload)
                    if retained_total > _MAX_RETAINED_TOTAL_BYTES:
                ...
                payloads[name] = payload
                attributes[name] = {
                    "kind": "file", "mode": mode or 0o644, "mtime": calendar.timegm((*member.date_time, 0, 0, 0)),
                    "size": member.file_size, "sha256": digest, "contains_cr": contains_cr, "content_findings": content_findings,
                }
    ...
    try:
        with tarfile.open(path, mode="r:*") as archive:
            ...
            if not member.isfile() or member.size != expected_size:
                raise ReleaseError(f"OCI descriptor payload is not the expected regular file: {name}")
            handle = archive.extractfile(member)
            if handle is None:
            ...
            if len(payload) != expected_size:
                raise ReleaseError(f"OCI descriptor payload is truncated or extended: {name}")
            return payload
    except (KeyError, OSError, tarfile.TarError) as error:
        raise ReleaseError(f"cannot read OCI descriptor payload {name}: {error}") from error
...
def _oci_descriptor(
    ...
    *,
    ...
    permitted = {expected_media_type} if isinstance(expected_media_type, str) else expected_media_type
    if media_type not in permitted:
        raise ReleaseError(f"OCI {label} descriptor has unsupported media type: {media_type}")
    ...
    if not isinstance(attribute, dict) or attribute.get("kind") != "file" or attribute.get("size") != size or attribute.get("sha256") != hexadecimal:
        raise ReleaseError(f"OCI {label} descriptor does not match its blob: {blob_name}")
    if not parse_json:
        return blob_name, None
    document = load_json_bytes(_read_tar_member_bytes(archive_path, blob_name, size), f"OCI {label} blob")
    ...
    try:
        with tarfile.open(archive_path, mode="r:*") as archive:
            ...
            if not member.isfile() or member.size != compressed_size:
                raise ReleaseError(f"OCI layer is not the expected regular blob: {blob_name}")
            handle = archive.extractfile(member)
            if handle is None:
            ...
            with handle, gzip.GzipFile(fileobj=handle, mode="rb") as decompressed:
                while chunk := decompressed.read(_READ_CHUNK_BYTES):
                    ...
                    digest.update(chunk)
    except (EOFError, gzip.BadGzipFile, KeyError, OSError, tarfile.TarError, zlib.error) as error:
        raise ReleaseError(f"cannot decompress OCI layer {blob_name}: {error}") from error
                ...
                with tarfile.open(fileobj=decompressed, mode="r|") as layer_archive:
                    for layer_member in layer_archive:
                        if len(names) >= maximum_entries:
                            raise ReleaseError(f"OCI layer entry count exceeds the contract limit: {blob_name}")
                        name = layer_member.name.rstrip("/") if layer_member.isdir() else layer_member.name
                        name = safe_relative_path(name)
                        if name in names:
                            raise ReleaseError(f"OCI layer contains a duplicate path: {name}")
                        names.add(name)
                        portable_key = name.casefold()
                        if portable_key in portable_names:
                            raise ReleaseError(
                        ...
                        if not layer_member.isfile() and not layer_member.isdir():
                            raise ReleaseError(f"OCI layer entry is not a regular file or directory: {name}")
                        if layer_member.uid != 0 or layer_member.gid != 0:
                            raise ReleaseError(f"OCI layer entry has a nonzero owner: {name}")
                        if layer_member.isdir():
                            if layer_member.mode != 0o755:
                                raise ReleaseError(f"OCI layer directory mode is not 0755: {name}")
                            continue
                        if layer_member.mode not in allowed_modes:
                            raise ReleaseError(f"OCI layer file mode is not allowlisted: {name}")
                        if layer_member.size < 0 or layer_member.size > maximum_member_bytes:
                            raise ReleaseError(f"OCI layer member exceeds the contract size limit: {name}")
                        expanded_member_bytes += layer_member.size
                        if expanded_member_bytes > maximum_uncompressed_bytes:
                            raise ReleaseError(f"OCI layer members exceed the uncompressed size limit: {blob_name}")
                        ...
                        if content_findings:
                            raise ReleaseError(f"OCI layer member failed content scan ({', '.join(content_findings)}): {name}")
                        if (Path(name).suffix.lower() in _TEXT_SUFFIXES or Path(name).name in _TEXT_NAMES) and contains_cr:
                            raise ReleaseError(f"OCI layer text member has non-LF line endings: {name}")
    except (EOFError, gzip.BadGzipFile, KeyError, OSError, tarfile.TarError, zlib.error) as error:
        raise ReleaseError(f"cannot inspect OCI layer tar {blob_name}: {error}") from error
...
def _validate_oci_layout(
    ...
    if not isinstance(index, dict) or not {"schemaVersion", "mediaType", "manifests"}.issubset(index) or not set(index).issubset({"schemaVersion", "mediaType", "manifests", "annotations"}):
        raise ReleaseError("OCI index has an unexpected shape")
    if index.get("schemaVersion") != 2 or index.get("mediaType") != "application/vnd.oci.image.index.v1+json":
        raise ReleaseError("OCI index identity is invalid")
    manifests = index.get("manifests")
    if not isinstance(manifests, list) or len(manifests) != 2:
        raise ReleaseError("OCI index must contain exactly linux/amd64 and linux/arm64 manifests")
    ...
    for descriptor in manifests:
        platform_value = descriptor.get("platform") if isinstance(descriptor, dict) else None
        if not isinstance(platform_value, dict) or set(platform_value) != {"os", "architecture"}:
        ...
        if not isinstance(platform_os, str) or not isinstance(platform_architecture, str):
        ...
        if platform_key not in expected_platforms or platform_key in observed_platforms:
            raise ReleaseError(f"OCI manifest platform is missing, duplicated, or unsupported: {platform_key}")
        observed_platforms.add(platform_key)
        annotations = descriptor.get("annotations")
        if not isinstance(annotations, dict):
            raise ReleaseError("OCI manifest descriptor annotations are missing")
        if expected_version is not None and annotations.get("org.opencontainers.image.version") != expected_version:
            raise ReleaseError(f"OCI manifest version annotation mismatch: {platform_key}")
        if expected_abi is not None and annotations.get("dev.cigar.context-abi") != expected_abi:
            raise ReleaseError(f"OCI manifest Context ABI annotation mismatch: {platform_key}")
        ...
        if not {"schemaVersion", "mediaType", "config", "layers"}.issubset(manifest) or not set(manifest).issubset({"schemaVersion", "mediaType", "config", "layers", "annotations"}):
        ...
        if config.get("os") != platform_key[0] or config.get("architecture") != platform_key[1]:
            raise ReleaseError(f"OCI config platform disagrees with its index descriptor: {platform_key}")
        ...
        if (
            ...
            raise ReleaseError(f"OCI runtime user is root or unspecified: {platform_key}")
        entrypoint = runtime.get("Entrypoint")
        command = runtime.get("Cmd")
        if not any(isinstance(value, list) and value and all(isinstance(item, str) and item for item in value) for value in (entrypoint, command)):
            raise ReleaseError(f"OCI runtime has no explicit entrypoint or command: {platform_key}")
        layers = manifest.get("layers")
        rootfs = config.get("rootfs")
        diff_ids = rootfs.get("diff_ids") if isinstance(rootfs, dict) and rootfs.get("type") == "layers" else None
        if not isinstance(layers, list) or not layers or not isinstance(diff_ids, list) or len(diff_ids) != len(layers):
            raise ReleaseError(f"OCI manifest layers and config diff IDs disagree: {platform_key}")
        for position, layer in enumerate(layers):
            ...
            if not isinstance(diff_ids[position], str) or re.fullmatch(r"sha256:[0-9a-f]{64}", diff_ids[position]) is None:
                raise ReleaseError(f"OCI config diff ID is invalid: {platform_key} #{position}")
            ...
            if not isinstance(layer_size, int) or isinstance(layer_size, bool):
                raise ReleaseError(f"OCI layer descriptor size is invalid: {platform_key} #{position}")
            ...
            if diff_ids[position] != layer_diff_ids[layer_blob]:
                raise ReleaseError(f"OCI config diff ID does not match the layer bytes: {platform_key} #{position}")
    ...
    blob_names = {name for name, attribute in attributes.items() if name.startswith("blobs/sha256/") and attribute.get("kind") == "file"}
    if blob_names != referenced_blobs:
        raise ReleaseError(f"OCI layout contains unreferenced or missing blobs: extra={sorted(blob_names - referenced_blobs)}, missing={sorted(referenced_blobs - blob_names)}")
    return {
...
def verify(
    ...
    if archive_format not in contract.get("formats", []):
        raise ReleaseError(f"archive format {archive_format} is not permitted by contract")
    ...
    if (
        not isinstance(maximum_entries, int) or isinstance(maximum_entries, bool) or maximum_entries <= 0
        or not isinstance(maximum_member, int) or isinstance(maximum_member, bool) or maximum_member <= 0
        or not isinstance(maximum_total, int) or isinstance(maximum_total, bool) or maximum_total <= 0
    ):
    ...
    if not allowed_modes or not allowed_modes.issubset({0o644, 0o755}) or contract.get("symlinks") != "forbid":
        raise ReleaseError("package contract mode or symlink policy is invalid")
    exemptions = contract.get("content_scan_exemptions", [])
    if (
        not isinstance(exemptions, list)
        or any(
            not isinstance(entry, dict)
            or set(entry) != {"pattern", "reason"}
            or not all(isinstance(value, str) and value for value in entry.values())
    ...
    if archive_format in {"tar", "tar.gz"}:
        payloads, attributes = _read_tar(
            archive_path, maximum_entries, maximum_member, maximum_total,
            retained_patterns, True, exemptions,
        )
    else:
    ...
    if contract.get("id") == "oci-image-v1":
        if archive_format != "tar":
            findings.append({"path": archive_path.name, "finding": "OCI layout must be an uncompressed outer tar"})
        else:
            try:
                layer_limit = contract.get("max_layer_uncompressed_bytes")
                if not isinstance(layer_limit, int) or isinstance(layer_limit, bool) or layer_limit <= 0:
    ...
    for name, attribute in attributes.items():
        directory_allowed = attribute["kind"] == "directory" and any(
            pattern.startswith(f"{name}/") or matches(f"{name}/placeholder", [pattern]) for pattern in allow
        )
        ...
        if attribute["kind"] == "file" and attribute["mode"] not in allowed_modes:
            findings.append({"path": name, "finding": f"mode-{attribute['mode']:04o}-not-allowed"})
        if attribute["kind"] == "directory" and attribute["mode"] != 0o755:
            findings.append({"path": name, "finding": f"directory-mode-{attribute['mode']:04o}-not-allowed"})
    ...
    for group in contract.get("required_any", []):
        if not isinstance(group, list) or not group or not any(value in names for value in group):
            findings.append({"path": "|".join(group) if isinstance(group, list) else "<invalid>", "finding": "required-choice-missing"})
    ...
    if "RELEASE-METADATA.json" in payloads:
        if metadata_payload is None:
            findings.append({"path": "RELEASE-METADATA.json", "finding": "release-metadata-exceeds-size-limit"})
            loaded = None
        else:
            loaded = load_json_bytes(metadata_payload, "RELEASE-METADATA.json")
        if not isinstance(loaded, dict) or loaded.get("schema_version") != "cigar.release-metadata.v1":
            findings.append({"path": "RELEASE-METADATA.json", "finding": "invalid-release-metadata"})
        else:
            metadata = loaded
            expected_metadata_keys = {
                "schema_version", "artifact_id", "product_version", "context_abi", "source_date_epoch",
                "source", "input_tree_sha256", "input_file_count", "contract", "contract_sha256"
            }
            if set(metadata) != expected_metadata_keys:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "unexpected-release-metadata-shape"})
            if expected_version is not None and metadata.get("product_version") != expected_version:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "product-version-mismatch"})
            if expected_abi is not None and metadata.get("context_abi") != expected_abi:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "context-abi-mismatch"})
            epoch = metadata.get("source_date_epoch")
            if isinstance(epoch, int):
                if expected_epoch is not None and epoch != expected_epoch:
                    findings.append({"path": "RELEASE-METADATA.json", "finding": "source-date-epoch-mismatch"})
                for name, attribute in attributes.items():
                    archive_epoch = epoch if archive_format not in {"zip", "wheel"} else epoch - (epoch % 2)
                    if "mtime" in attribute and attribute["mtime"] != archive_epoch:
                        findings.append({"path": name, "finding": "nondeterministic-timestamp"})
            else:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "invalid-source-date-epoch"})
            if metadata.get("contract_sha256") != sha256_file(contract_path):
                findings.append({"path": "RELEASE-METADATA.json", "finding": "package-contract-digest-mismatch"})
            try:
                safe_relative_path(metadata.get("contract", ""))
            except ReleaseError:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "unsafe-package-contract-reference"})
            source = metadata.get("source")
            if not isinstance(source, dict) or set(source) != {"revision", "tree_sha256", "committed", "clean"}:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "invalid-source-identity"})
            elif not isinstance(source.get("revision"), str) or not isinstance(source.get("committed"), bool) or not isinstance(source.get("clean"), bool):
                findings.append({"path": "RELEASE-METADATA.json", "finding": "invalid-source-identity-types"})
            elif not isinstance(source.get("tree_sha256"), str) or len(source["tree_sha256"]) != 64:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "invalid-source-tree-digest"})
            input_digest = hashlib.sha256()
            input_files = sorted(name for name in payloads if name != "RELEASE-METADATA.json")
            ...
            if metadata.get("input_tree_sha256") != calculated_input_digest:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "input-tree-digest-mismatch"})
            if metadata.get("input_file_count") != len(input_files):
                findings.append({"path": "RELEASE-METADATA.json", "finding": "input-file-count-mismatch"})
            if metadata.get("artifact_id") == "source" and isinstance(source, dict) and source.get("tree_sha256") != calculated_input_digest:
                findings.append({"path": "RELEASE-METADATA.json", "finding": "source-tree-digest-mismatch"})
    elif expected_epoch is not None:
        archive_epoch = expected_epoch if archive_format not in {"zip", "wheel"} else expected_epoch - (expected_epoch % 2)
        for name, attribute in attributes.items():
            if "mtime" in attribute and attribute["mtime"] != archive_epoch:
                findings.append({"path": name, "finding": "nondeterministic-timestamp"})
    ...
    if contract.get("line_endings") != "lf":
        findings.append({"path": str(contract_path), "finding": "unsupported-line-ending-policy"})
    else:
        for name in payloads:
            if Path(name).suffix.lower() in _TEXT_SUFFIXES or Path(name).name in _TEXT_NAMES:
    ...
    try:
        if "version_binding" in contract and expected_version is not None:
            if _binding_value(payloads, contract["version_binding"], "version") != expected_version:
                findings.append({"path": contract["version_binding"]["path_pattern"], "finding": "product-version-mismatch"})
        if "abi_binding" in contract and expected_abi is not None:
            if _binding_value(payloads, contract["abi_binding"], "Context ABI") != expected_abi:
                findings.append({"path": contract["abi_binding"]["path_pattern"], "finding": "context-abi-mismatch"})
    except ReleaseError as error:
        findings.append({"path": str(contract_path), "finding": str(error)})

    if findings:
        summary = "; ".join(f"{item['path']}: {item['finding']}" for item in findings[:20])
        if len(findings) > 20:
    ...
    return {
        ...
        "status": "passed",
        "archive": {"name": archive_path.name, "sha256": sha256_file(archive_path), "bytes": archive_path.stat().st_size},
...
def main() -> int:
    ...
    if arguments.report is not None:
        require_distinct_output(arguments.report.resolve(), [arguments.archive, arguments.contract], "package verification")
    report = verify(
        arguments.archive.resolve(), arguments.contract.resolve(), arguments.expected_version,
        arguments.expected_abi, arguments.source_date_epoch
    )
    if arguments.report is not None:

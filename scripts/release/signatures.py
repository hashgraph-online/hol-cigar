...
from release_lib import ReleaseError, canonical_json_bytes, load_json, process_failure_summary, run_bounded, safe_relative_path, sha256_file, write_bytes, write_json
...
def parse_arguments() -> argparse.Namespace:
    ...
    sign = subparsers.add_parser("sign", help="sign a payload with an existing Ed25519 private PEM")
    ...
    sign.add_argument("--signed-at", type=int, required=True, help="Unix timestamp recorded in the signed envelope")
    sign.add_argument("--expires-at", type=int, help="optional exclusive Unix expiry timestamp")
    sign.add_argument("--out", type=Path, required=True)
    verify = subparsers.add_parser("verify", help="verify a signature envelope with a trusted public PEM")
    ...
    verify.add_argument("--verification-time", type=int, help="Unix time used for expiry checks; defaults to the current time")
    return parser.parse_args()


def _run(arguments: list[str]) -> bytes:
    result = run_bounded(arguments, timeout=30, max_stdout=1024 * 1024, max_stderr=1024 * 1024)
...
def _public_der(public_key: Path) -> bytes:
    payload = _run(["openssl", "pkey", "-pubin", "-in", str(public_key), "-outform", "DER"])
    if len(payload) != len(_ED25519_SPKI_PREFIX) + 32 or not payload.startswith(_ED25519_SPKI_PREFIX):
        raise ReleaseError("release public key is not an Ed25519 SubjectPublicKeyInfo key")
    return payload
...
def _timestamp(value: Any, label: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or value < 0 or value > 253_402_300_799:
        raise ReleaseError(f"signature {label} is not a valid Unix timestamp")
    return value


def _identity(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value or len(value.encode("utf-8")) > 256 or value != value.strip():
...
def _unsigned_envelope(value: Any) -> dict[str, Any]:
    ...
    required = {
        "schema_version", "algorithm", "key_id", "signer_principal", "purpose", "signed_at", "payload"
    }
    ...
    if value.get("schema_version") != "cigar.signature-envelope.v1" or value.get("algorithm") != "Ed25519":
...
def sign(
    ...
    *,
    ...
    if output.resolve() in resolved_inputs:
        raise ReleaseError("signature output must not replace its payload or key material")
    ...
    unsigned: dict[str, Any] = {
        ...
        "purpose": purpose,
        "signed_at": signed_at,
        "payload": {"name": payload.name, "sha256": sha256_file(payload), "bytes": payload.stat().st_size},
    }
    ...
    with tempfile.TemporaryDirectory(prefix="cigar-signature-") as directory:
        ...
        _run(["openssl", "pkeyutl", "-sign", "-rawin", "-inkey", str(private_key), "-in", str(signature_input), "-out", str(signature)])
        _run(["openssl", "pkeyutl", "-verify", "-rawin", "-pubin", "-inkey", str(public_key), "-in", str(signature_input), "-sigfile", str(signature)])
        signature_bytes = signature.read_bytes()
    if unsigned["payload"]["sha256"] != sha256_file(payload) or unsigned["payload"]["bytes"] != payload.stat().st_size:
    ...
    envelope = {**unsigned, "signature_base64": base64.b64encode(signature_bytes).decode("ascii")}
...
def verify(
    ...
    *,
    ...
    unsigned = _unsigned_envelope({key: value for key, value in envelope.items() if key != "signature_base64"})
    ...
    if unsigned["purpose"] != expected_purpose:
        raise ReleaseError(f"signature purpose is {unsigned['purpose']}, expected {expected_purpose}")
    ...
    verification_time = int(time.time()) if verification_time is None else _timestamp(verification_time, "verification-time")
    ...
    if reference.get("name") != payload.name or reference.get("sha256") != observed_digest or reference.get("bytes") != observed_size:
    ...
    with tempfile.TemporaryDirectory(prefix="cigar-signature-") as directory:
        ...
        _run(["openssl", "pkeyutl", "-verify", "-rawin", "-pubin", "-inkey", str(public_key), "-in", str(signature_input), "-sigfile", str(signature_path)])
    if sha256_file(payload) != observed_digest or payload.stat().st_size != observed_size:

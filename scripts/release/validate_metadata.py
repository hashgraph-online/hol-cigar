...
from release_lib import ReleaseError, load_json, repo_root, resolve_beneath, sha256_file, validate_qualification_policy, validate_release_policy_documents
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--release", action="store_true", help="also require external candidate prerequisites")
...
def _require_string_list(value: Any, label: str, *, nonempty: bool = False) -> list[str]:
    if not isinstance(value, list) or (nonempty and not value) or not all(isinstance(item, str) and item for item in value):
        raise ReleaseError(f"{label} must be {'a non-empty ' if nonempty else ''}list of strings")
    return value
...
def main() -> int:
    ...
    supported_targets = set(support["targets"]["tier1"]) | set(support["targets"]["tier2"])
    ...
    for entry in artifacts:
        ...
        required_contract_keys = {
            ...
            "modes", "max_entries", "max_member_bytes", "max_total_bytes", "content_scan", "content_scan_exemptions",
        }
        ...
        if not required_contract_keys.issubset(contract) or not set(contract).issubset(required_contract_keys | optional_contract_keys):
            raise ReleaseError(f"artifact {entry['id']} contract has missing or unexpected fields")
        _require_string_list(contract.get("formats"), f"{contract_path}: formats", nonempty=True)
        _require_string_list(contract.get("allow"), f"{contract_path}: allow", nonempty=True)
        _require_string_list(contract.get("deny"), f"{contract_path}: deny")
        _require_string_list(contract.get("modes"), f"{contract_path}: modes", nonempty=True)
        limits = [contract.get("max_entries"), contract.get("max_member_bytes"), contract.get("max_total_bytes")]
        if not all(isinstance(value, int) and not isinstance(value, bool) and value > 0 for value in limits) or contract["max_member_bytes"] > contract["max_total_bytes"]:
            raise ReleaseError(f"artifact {entry['id']} contract has invalid resource limits")
        if contract.get("symlinks") != "forbid" or contract.get("line_endings") != "lf" or contract.get("content_scan") is not True:
            raise ReleaseError(f"artifact {entry['id']} contract weakens a mandatory package policy")
        ...
        if contract.get("id") == "oci-image-v1":
            ...
            raise ReleaseError(f"non-OCI artifact {entry['id']} declares an OCI layer limit")
        exemptions = contract.get("content_scan_exemptions")
        if not isinstance(exemptions, list) or any(
            not isinstance(value, dict) or set(value) != {"pattern", "reason"}
            or not all(isinstance(item, str) and item for item in value.values())
            for value in exemptions
        ):
            raise ReleaseError(f"artifact {entry['id']} contract has invalid content-scan exemptions")
        platform = entry.get("platform")
        if entry.get("kind") == "binary-archive" and platform not in supported_targets:
            raise ReleaseError(f"binary artifact {entry['id']} claims unsupported target {platform}")
    discovered_contract_paths = {path.resolve() for path in (root / "packaging/contracts").glob("*.json")}
    if discovered_contract_paths != referenced_contract_paths:
        missing = sorted(path.name for path in referenced_contract_paths - discovered_contract_paths)
        orphaned = sorted(path.name for path in discovered_contract_paths - referenced_contract_paths)
        raise ReleaseError(f"package contract inventory mismatch; missing={missing}, orphaned={orphaned}")
    ...
    for entry in local.get("archives", []):
        ...
        if identifier not in matrix_by_id:
            raise ReleaseError(f"local archive {identifier} is absent from artifact matrix")
        if entry.get("filename") != matrix_by_id[identifier].get("filename"):
            raise ReleaseError(f"local archive {identifier} filename disagrees with artifact matrix")
        resolve_beneath(root, entry["contract"])
        _require_string_list(entry.get("include"), f"local archive {identifier} include", nonempty=True)
    ...
    pnpm_mismatch = root_package_manager != expected_pnpm or sdk_package_manager != expected_pnpm
    if arguments.release and pnpm_mismatch:
        raise ReleaseError(f"pnpm pin mismatch: support={expected_pnpm}, root={root_package_manager}, sdk={sdk_package_manager}")

    requirements = load_json(root / "packaging/release-requirements.v1.json")
    categories = _require_string_list(requirements.get("required_evidence_categories"), "required evidence categories", nonempty=True)
    ...
    if not set(signed_categories).issubset(category_set):
        raise ReleaseError("directly signed evidence categories are not required evidence categories")
    ...
    for gate in gates:
        required_gate_keys = {"name", "category", "aggregation", "comparison", "threshold"}
        allowed_gate_keys = required_gate_keys | {"valid_min", "valid_max"}
        if not isinstance(gate, dict) or not required_gate_keys.issubset(gate) or not set(gate).issubset(allowed_gate_keys):
            raise ReleaseError("release metric gate has an unexpected shape")
        if gate.get("category") not in category_set or gate.get("aggregation") not in {"max", "min", "sum"} or gate.get("comparison") not in {"gte", "lte"}:
            raise ReleaseError("release metric gate has an invalid category or operation")
        if not isinstance(gate.get("name"), str) or not gate["name"] or not isinstance(gate.get("threshold"), (int, float)) or isinstance(gate["threshold"], bool):
        ...
        if gate.get("valid_min") is not None and gate.get("valid_max") is not None and gate["valid_min"] > gate["valid_max"]:
    ...
    qualification_map = load_json(resolve_beneath(root, requirements["qualification_category_map"]))
    ...
    used_qualifications = {value for entry in artifacts for value in entry.get("qualification", [])}
    if not used_qualifications.issubset(mappings):
        raise ReleaseError(f"artifact qualifications are unmapped: {sorted(used_qualifications - set(mappings))}")
    for name, specification in mappings.items():
        requirements_list = specification.get("requirements", [specification]) if isinstance(specification, dict) else []
        ...
        for mapped in requirements_list:
            if not isinstance(mapped, dict) or mapped.get("category") not in category_set or not isinstance(mapped.get("check"), str):
                raise ReleaseError(f"qualification mapping references an unknown category: {name}")
    ...
    for mapped in universal:
        if not isinstance(mapped, dict) or set(mapped) != {"category", "check"} or mapped.get("category") not in category_set or not isinstance(mapped.get("check"), str):
    ...
    for mapped in qualification_map.get("additional_requirements", []):
        if not isinstance(mapped, dict) or mapped.get("artifact_kind") not in artifact_kinds or mapped.get("category") not in category_set:
    ...
    required_release_schemas = {
        "artifact-matrix.v1.schema.json", "docs-check.v1.schema.json", "install-qualification.v1.schema.json",
        "installed-driver.v1.schema.json", "operation-exercise-summary.v1.schema.json",
        "operation-exercise.v1.schema.json", "package-contract.v1.schema.json", "provenance.v1.schema.json",
        "qualification-evidence.v1.schema.json", "release-build.v1.schema.json",
        "release-evidence.v1.schema.json", "release-metadata.v1.schema.json",
        "release-trust-policy.v1.schema.json", "release-verification.v1.schema.json",
        "reproducibility-report.v1.schema.json", "sbom-artifacts.v1.schema.json",
        "signature-envelope.v1.schema.json", "wp21-local-qualification.v1.schema.json",
    }
    schema_paths = sorted((root / "packaging/schemas").glob("*.json"))
    if {path.name for path in schema_paths} != required_release_schemas:
        missing = sorted(required_release_schemas - {path.name for path in schema_paths})
        extra = sorted({path.name for path in schema_paths} - required_release_schemas)
        raise ReleaseError(f"release schema inventory mismatch; missing={missing}, extra={extra}")
    ...
    for required in _require_string_list(docs.get("required_pages"), "required documentation pages", nonempty=True):
        resolve_beneath(root, required)

    license_path = root / "packaging/licenses/Apache-2.0.txt"
    if "Apache License" not in license_path.read_text(encoding="utf-8") or sha256_file(license_path) == "0" * 64:
    ...
    if inventory.get("schema_version") != "cigar.third-party-license-inventory.v1" or inventory.get("policy_sha256") != sha256_file(license_policy_path):
        raise ReleaseError("third-party license inventory is missing or stale")
    if not isinstance(inventory.get("component_count"), int) or inventory["component_count"] <= 0:
    ...
    if arguments.release:
        ...
        if inventory.get("review_required_count") != 0 or inventory.get("status") != "complete":
            raise ReleaseError("release requires a fully reviewed third-party license inventory")
        blocking_gaps = [entry["id"] for entry in gaps if entry.get("release_blocking") is True]
        if blocking_gaps:
    ...
    print(f"validated {len(artifacts)} artifacts, {len(set(entry['contract'] for entry in artifacts))} contracts, {len(categories)} evidence categories, version {version}, ABI {abi}{suffix}")
    return 0
...
if __name__ == "__main__":
    ...
    except (OSError, KeyError, TypeError, json.JSONDecodeError, tomllib.TOMLDecodeError, ReleaseError) as error:

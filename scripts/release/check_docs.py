...
_DIRECTIVE = re.compile(r"^<!--\s*docs-check:\s*(command|illustrative)\s*([a-z0-9._-]+)?\s*-->$")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--execute", action="append", choices=["local", "installed-candidate", "live"], default=[])
    parser.add_argument("--execute-local", action="store_true")
    parser.add_argument("--variables", type=Path, help="strict JSON mapping used to expand ${NAME}")
...
def _published_files(root: Path, manifest: dict[str, Any]) -> list[Path]:
    ...
    files = [path for relative, path in expand_files(root, includes, []) if relative.endswith(".md")]
    if not files:
...
def _check_links(root: Path, files: list[Path], assets: set[str]) -> tuple[int, set[str]]:
    ...
    for path in files:
        ...
        for raw in _LINK.findall(text):
            ...
            try:
                ...
                errors.append(f"{path.relative_to(root)}: missing link target {target_text}")
                continue
            if resolved != root and root not in resolved.parents:
                errors.append(f"{path.relative_to(root)}: link escapes repository {target_text}")
                continue
            relative = resolved.relative_to(root).as_posix()
            if resolved not in published and relative not in assets:
                errors.append(f"{path.relative_to(root)}: link target is not a published page or allowlisted asset {target_text}")
                continue
            if relative in assets:
                referenced_assets.add(relative)
            if separator and fragment:
                if resolved.suffix.lower() != ".md":
                    errors.append(f"{path.relative_to(root)}: fragment on non-Markdown target {target_text}")
                else:
                    target_anchors = anchors.get(resolved, _anchors(resolved))
                    if fragment not in target_anchors:
                        errors.append(f"{path.relative_to(root)}: missing anchor {target_text}")
            count += 1
    if errors:
...
def _check_blocks(root: Path, files: list[Path], commands: dict[str, dict[str, Any]]) -> tuple[int, set[str]]:
    blocks = 0
    ...
    for path in files:
        ...
        while index < len(lines):
            ...
            while index < len(lines) and lines[index] != "```":
                ...
                index += 1
            if index == len(lines):
                errors.append(f"{path.relative_to(root)}:{start + 1}: unclosed code fence")
                break
            blocks += 1
            ...
            if language in _SHELL_LANGUAGES:
                ...
                if directive is None:
                    errors.append(f"{path.relative_to(root)}:{start + 1}: shell block has no docs-check directive")
                elif directive.group(1) == "command":
                    command_id = directive.group(2)
                    if command_id not in commands:
                        errors.append(f"{path.relative_to(root)}:{start + 1}: unknown docs command {command_id}")
                    elif command_id in referenced:
                        errors.append(f"{path.relative_to(root)}:{start + 1}: docs command is referenced by multiple blocks: {command_id}")
                    else:
                        referenced.add(command_id)
                elif directive.group(2) is not None:
                    errors.append(f"{path.relative_to(root)}:{start + 1}: illustrative block must not name a command")
            payload = "\n".join(content)
            if command_id in commands and sha256_bytes(payload.encode("utf-8")) != commands[command_id]["block_sha256"]:
                errors.append(f"{path.relative_to(root)}:{start + 1}: shell block differs from docs command {command_id}")
            try:
                if language == "json":
                ...
                errors.append(f"{path.relative_to(root)}:{start + 1}: invalid {language} block: {error}")
            index += 1
    if errors:
        raise ReleaseError("; ".join(errors[:30]))
...
def _validate_commands(value: Any) -> dict[str, dict[str, Any]]:
    ...
    for command in value:
        ...
        expected_keys = {"id", "block_sha256", "mode", "cwd", "steps"} if has_steps else {
            "id", "block_sha256", "mode", "cwd", "argv", "expected_exit",
        }
        if set(command) != expected_keys:
            raise ReleaseError(f"documentation command has an unexpected shape: {command.get('id')}")
        ...
        if (
            ...
            raise ReleaseError(f"documentation command identity, mode, cwd, or block binding is invalid: {identifier}")
        steps = command.get("steps") if has_steps else [{"argv": command.get("argv"), "expected_exit": command.get("expected_exit")}]
        if not isinstance(steps, list) or not steps:
            raise ReleaseError(f"documentation command has no executable steps: {identifier}")
        for step in steps:
            if not isinstance(step, dict) or set(step) != {"argv", "expected_exit"}:
                raise ReleaseError(f"documentation command step has an unexpected shape: {identifier}")
            argv = step.get("argv")
            ...
            if (
                ...
                raise ReleaseError(f"documentation command argv or expected exit is invalid: {identifier}")
...
def _expand(value: str, variables: dict[str, str]) -> str:
    ...
    if len(expanded.encode("utf-8")) > 4096 or any(
        ...
        raise ReleaseError("expanded documentation command value is invalid or unbounded")
    return expanded


def _execute(root: Path, commands: list[dict[str, Any]], modes: set[str], variables: dict[str, str]) -> tuple[int, int]:
    executed = 0
    failed = 0
    for command in commands:
        ...
        if steps is None:
            steps = [{"argv": command.get("argv"), "expected_exit": command.get("expected_exit", 0)}]
        if not isinstance(steps, list) or not steps:
            raise ReleaseError(f"documentation command {command.get('id')} has no executable steps")
        for step in steps:
            argv = step.get("argv")
            if not isinstance(argv, list) or not argv or not all(isinstance(item, str) for item in argv):
                raise ReleaseError(f"documentation command {command.get('id')} argv is invalid")
            ...
            if result.returncode != step.get("expected_exit", 0):
                failed += 1
                raise ReleaseError(process_failure_summary(result, f"documentation command {command['id']}"))
...
def main() -> int:
    ...
    for required in required_pages:
        if resolve_beneath(root, required) not in published:
            raise ReleaseError(f"required documentation page is not published: {required}")
    assets = _site_assets(root, manifest)
    links, referenced_assets = _check_links(root, files, assets)
    if referenced_assets != assets:
        raise ReleaseError(f"documentation asset allowlist contains unreferenced entries: {sorted(assets - referenced_assets)}")
    blocks, referenced = _check_blocks(root, files, commands)
    unreferenced = set(commands) - referenced
    if unreferenced:
        raise ReleaseError(f"documentation commands are not referenced by a published page: {sorted(unreferenced)}")
    ...
    if arguments.variables is not None:
        ...
        if (
            ...
            raise ReleaseError("documentation command variables must be a string mapping")
        variables = loaded
    ...
    print(f"documentation passed: {len(files)} pages, {links} links, {blocks} code blocks, {executed} executed command steps")
    return 0

...
The GitHub shadow lane is intentionally keyless and does not execute candidate code. It emits only
an unsigned `disabled` status receipt until a separately pinned trusted controller can isolate
candidate execution from evidence-signing authority. Exact-source shadow qualification continues
through the local private workflow in the meantime.
...
2. Create GitHub environments with the same names. Configure required reviewers and prevent
   ...
   API before rollout.
3. Give development only `CIGAR_DEVELOPMENT_API_KEY`. Give the current GitHub shadow environment
   no credentials or private corpus mounts. A future trusted shadow controller may receive
   candidate-scoped mounts and signing authority only after it proves candidate processes cannot
   read or invoke them. Give promotion only
